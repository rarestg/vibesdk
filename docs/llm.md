...
const tokenData = await provider.exchangeCodeForToken(code, storedState.codeVerifier);
...
const csrfToken = await crypto.subtle.digest('SHA-256', crypto.getRandomValues(new Uint8Array(32)));
```
