...
function displayTopModels(models: ModelSummary[], topN: number): void {
  ...
  const sortedModels = models.sort((a, b) => b.tokensIn + b.tokensOut - (a.tokensIn + a.tokensOut)).slice(0, topN);
  ...
  results.forEach((result) => {
    ...
    if (result.name.includes('Provider Request Analytics')) {
      ...
      if (totalRequests) {
        ...
        const cacheHitRate =
          totalRequests.count > 0 ? ((cachedRequests / totalRequests.count) * 100).toFixed(2) : '0.00';
        ...
        queries.push(
          {
            ...
            query: QUERIES.providerRequests(timeRange.start, timeRange.end, granularity),
          },
          {
            name: 'User Model Token Analytics',
            query: QUERIES.modelTokens(timeRange.start, timeRange.end, granularity),
          },
        );
      }
    } else if (args.chatId) {
        ...
        queries.push(
          {
            ...
            query: QUERIES.chatProviderRequests(timeRange.start, timeRange.end, args.chatId, granularity),
          },
          {
            name: `Chat Model Token Analytics (${args.chatId})`,
            query: QUERIES.chatModelTokens(timeRange.start, timeRange.end, args.chatId, granularity),
          },
        );
      }
    } else {
