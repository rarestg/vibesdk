...
export default function AppView() {
  ...
  const fetchAppDetails = useCallback(async () => {
    ...
    try {
      ...
      if (appResponse.success && appResponse.data) {
        ...
        throw new Error(appResponse.error?.message || 'Failed to fetch app details');
      }
    } catch (err) {
  ...
  const handleToggleVisibility = async () => {
    ...
    try {
      ...
      const newVisibility = app.visibility === 'private' ? 'public' : 'private';

      const response = await apiClient.updateAppVisibility(app.id, newVisibility);

      if (response.success && response.data) {
        // Update the app state with new visibility
        setApp((prev) => (prev ? { ...prev, visibility: newVisibility } : null));

        toast.success(response.data.message || `App is now ${newVisibility === 'private' ? 'private' : 'public'}`);
      } else {
        ...
        <Card className="max-w-md">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-text-tertiary mb-4">{error || "The app you're looking for doesn't exist."}</p>
  ...
  return (
    <div className="min-h-screen bg-bg-3 flex flex-col">
      <div className="container mx-auto px-4 pb-6 space-y-6 flex flex-col flex-1">
        {/* Back button */}
        <button onClick={() => history.back()} className="gap-2 flex items-center text-text-primary/80">
          ...
          <div className="flex-1">
            <div className="flex rounded w-fit pb-3 pt-2 flex-col mb-6">
              <div className="flex items-center gap-3 mb-2">
                <h1 className="text-4xl font-semibold tracking-tight text-text-primary">{app.title}</h1>
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant="outline" size="sm" onClick={handleFavorite} className={cn('gap-2 text-text-primary')}>
                  <Bookmark className={cn('h-4 w-4', isFavorited && 'fill-current')} />
                  {isFavorited ? 'Bookmarked' : 'Bookmark'}
                </Button>

                <Button variant="outline" size="sm" onClick={handleStar} className={cn('gap-2 text-text-primary')}>
                ...
                {app.githubRepositoryUrl && (
                  ...
                  >
                    ...
                    View on GitHub
                    {app.githubRepositoryVisibility === 'private' && <Lock className="h-3 w-3 opacity-70" />}
                  </Button>
                )}
            ...
            {app.description && <p className="text-text-primary my-3 max-w-4xl">{app.description}</p>}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col flex-1 gap-2">
          ...
          <div className="flex items-center gap-4">
            ...
            <TabsList className="inline-flex h-auto w-fit items-center gap-0.5 bg-bg-2 dark:bg-bg-1 rounded-md p-0.5 border border-border-primary/30">
              ...
              >
                <MessageSquare
                  className={cn('h-3.5 w-3.5 mr-1.5', activeTab === 'prompt' ? 'text-accent' : 'text-accent/60')}
                />
                Prompt
              </TabsTrigger>
            </TabsList>
            ...
            <Card className="px-2">
              <CardHeader className="overflow-hidden rounded-t">
                <div className="flex items-center gap-4 min-w-0">
                  <CardTitle className="text-base flex-shrink-0">Live Preview</CardTitle>
                  {/* Preview URL action buttons */}
                  {appUrl && (
                    <div className="ml-auto flex items-center gap-0 flex-shrink-0">
                      <Button variant="ghost" size="sm" onClick={handleCopyUrl} className="gap-2">
                        {urlCopied ? (
                          <>
                      ...
                      <Button variant="ghost" size="sm" onClick={() => window.open(appUrl, '_blank')} className="gap-2">
                    ...
                    <div className="relative w-full h-[400px] bg-gray-50 flex items-center justify-center">
                      ...
                      <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center z-10">
                        <div className="text-center p-8">
                          <h3 className="text-xl font-semibold mb-2 text-gray-700">Run App</h3>
                          <p className="text-gray-500 mb-6 max-w-md">Run the app to see a live preview.</p>
                          {deploymentProgress && <p className="text-sm text-gray-800 mb-4">{deploymentProgress}</p>}
                          <div className="flex gap-3 justify-center">
                            <Button onClick={handlePreviewDeploy} disabled={isDeploying} className="gap-2">
                              {isDeploying ? (
                                <>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    {app?.agentSummary && (
                      <p className="text-sm text-muted-foreground">{files.length} files generated</p>
                    )}
                  </div>
                  {activeFile && (
                    <Button
                      ...
                      <div className="flex-1 flex flex-col">
                        {activeFile ? (
                          <>
                            <div className="flex items-center justify-between p-3 border-b bg-bg-3">
                              <div className="flex items-center gap-2 flex-1">
                                <Code2 className="h-4 w-4" />
                                <span className="text-sm font-mono">{activeFile.filePath}</span>
                                {activeFile.explanation && (
                                  <span className="text-xs text-text-tertiary ml-3">{activeFile.explanation}</span>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="flex-1 flex items-center justify-center">
                            <p className="text-text-tertiary">Select a file to view</p>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <TabsContent value="prompt" className="flex-1">
            <Card>
              <CardHeader>
                ...
                <CardDescription>The initial prompt used to create this app</CardDescription>
              </CardHeader>
              <CardContent>
                {app?.agentSummary?.query || app?.originalPrompt ? (
                  ...
                  <div className="flex items-center justify-center py-12 text-text-tertiary">
                    ...
                    <p>{app?.agentSummary === null ? 'Loading prompt...' : 'No prompt available'}</p>
                  </div>
                )}
              </CardContent>
            </Card>
