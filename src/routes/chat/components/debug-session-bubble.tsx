...
export function DebugSessionBubble({ message, isActive, elapsedSeconds, toolCallCount }: DebugSessionBubbleProps) {
      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5 flex-1 min-w-0">
            ...
            >
              {isActive ? 'Deep Debugging' : hasError ? 'Debugging Failed' : 'Debugging Complete'}
            </span>

            {/* Timer and tool count */}
            <div className="flex items-center gap-2 ml-auto text-xs text-text-tertiary">
              {isActive && <span className="font-mono tabular-nums">{formatElapsedTime(elapsedSeconds)}</span>}
              {toolCallCount > 0 && (
                <span className="px-1.5 py-0.5 rounded bg-bg-3/50 font-medium">{toolCallCount}</span>
              )}
            </div>
          </div>

          {/* Expand/collapse icon */}
          <motion.div animate={{ rotate: isExpanded ? 180 : 0 }} transition={{ duration: 0.2 }} className="ml-2">
          ...
          {isExpanded && (
            ...
            >
              <div className="relative">
                ...
                <div ref={scrollAreaRef} onScroll={handleScroll} className="max-h-[600px] overflow-y-auto px-4 py-3">
                    ...
                    {message.ui?.toolEvents && message.ui.toolEvents.length > 0 && (
                      <div className="flex flex-col gap-1.5">
                        {message.ui.toolEvents
                          ...
                          .map((event, idx) => (
                            <ToolStatusIndicator key={`${event.name}-${event.timestamp}-${idx}`} event={event} />
                          ))}
                      </div>
                    )}
                  </div>
