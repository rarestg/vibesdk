...
export function Terminal({ logs, onCommand, isConnected, className }: TerminalProps) {
  ...
  return (
    <div className={clsx('flex flex-col h-full font-mono text-sm', 'bg-white dark:bg-[#1d1e1e]', className)}>
      {/* Terminal Output */}
      <div className="flex-1 min-h-0 overflow-hidden bg-gray-25 dark:bg-[#1d1e1e]">
        <ScrollArea ref={scrollAreaRef} className="h-full terminal-scroll">
          <div className="p-4 space-y-2">
            {logs.length === 0 ? (
              ...
              >
                ...
                <p className="text-sm font-medium mb-1">Terminal Ready</p>
              ...
              logs.map((log) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    >
                      {log.type === 'command' && <span className="text-[#f6821f] font-semibold mr-1">$</span>}
                      {log.type === 'stderr' && <span className="text-red-500 dark:text-red-400 mr-1">❌</span>}
                      {log.type === 'error' && <span className="text-red-500 dark:text-red-400 mr-1">🚫</span>}
                      {log.type === 'warn' && <span className="text-amber-500 dark:text-yellow-400 mr-1">⚠️</span>}
                      {log.type === 'info' && <span className="text-blue-500 dark:text-blue-400 mr-1">ℹ️</span>}
                      {log.type === 'debug' && <span className="text-gray-500 dark:text-gray-400 mr-1">🐛</span>}
                      {log.content}
                    </div>
                  </div>
                </div>
          ...
          <div className="flex items-center gap-2">
            <span
              className={clsx(
                ...
                isConnected ? 'text-[#f6821f]' : 'text-gray-400 dark:text-gray-600',
              )}
            >
              $
            </span>
