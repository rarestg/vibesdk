...
class DebugPanelErrorBoundary extends Component<{ children: React.ReactNode }, { hasError: boolean; error?: Error }> {
        ...
        <div className="flex items-center justify-between p-4 border-b border-border-primary bg-gradient-to-r from-muted to-accent">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            ...
            <div className="flex items-center gap-2 basis-full justify-end pt-2">
              <Button onClick={downloadDump} disabled={isGeneratingDump} title="Download debug dump">
                <Download className="w-4 h-4" />
                {isGeneratingDump ? 'Generating...' : 'Download'}
              </Button>
              <Button onClick={emailDump} disabled={isGeneratingDump} title="Email debug dump to developers">
          ...
          <div className="flex gap-1 flex-wrap">
            {[
              ...
              { key: 'warning' as const, label: 'Warnings', count: warningCount },
              {
                ...
                label: 'WebSocket',
                count: messages.filter((m) => m.type === 'websocket').length,
              },
              {
                key: 'info' as const,
                label: 'Info',
                count: messages.filter((m) => m.type === 'info').length,
              },
            ].map(({ key, label, count }) => (
              <button
                key={key}
                ...
                {[
                  ...
                  },
                  {
                    ...
                    count: messages.filter(
                      (m) =>
                        m.type === 'websocket' &&
                        (m.wsCategory === 'generation' || categorizeWebSocketMessage(m.messageType) === 'generation'),
                    ).length,
                  },
                  {
                    key: 'phase' as const,
                    ...
                    count: messages.filter(
                      (m) =>
                        m.type === 'websocket' &&
                        (m.wsCategory === 'phase' || categorizeWebSocketMessage(m.messageType) === 'phase'),
                    ).length,
                  },
                  {
                    key: 'file' as const,
                    ...
                    count: messages.filter(
                      (m) =>
                        m.type === 'websocket' &&
                        (m.wsCategory === 'file' || categorizeWebSocketMessage(m.messageType) === 'file'),
                    ).length,
                  },
                  {
                    ...
                    count: messages.filter(
                      (m) =>
                        m.type === 'websocket' &&
                        (m.wsCategory === 'deployment' || categorizeWebSocketMessage(m.messageType) === 'deployment'),
                    ).length,
                  },
                  {
                    key: 'system' as const,
                    ...
                    count: messages.filter(
                      (m) =>
                        m.type === 'websocket' &&
                        (m.wsCategory === 'system' || categorizeWebSocketMessage(m.messageType) === 'system'),
                    ).length,
                  },
                ].map(({ key, label, count }) => (
                  <button
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-bg-3/50 p-4 rounded-lg">
                      ...
                      <div className="space-y-1 text-sm">
                        <div>
                          Average: <span className="font-mono">{analyticsData.intervals.avg}</span>
                        </div>
                        <div>
                          Median: <span className="font-mono">{analyticsData.intervals.median}</span>
                        </div>
                        <div>
                          P99: <span className="font-mono">{analyticsData.intervals.p99}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-amber-50 p-4 rounded-lg">
                      ...
                      <div className="text-2xl font-bold text-amber-600">
                        {analyticsData.errorRate === '0'
                          ? '100'
                          : (100 - parseFloat(analyticsData.errorRate)).toFixed(0)}
                        %
                      </div>
                            ...
                            <div className="space-y-1 text-sm">
                              <div>
                                ...
                                <span className="font-mono font-medium">
                                  {analyticsData.operations.fileGeneration.linesPerSecond.avg.toFixed(1)} lines/s
                                </span>
                              </div>
                              <div>
                                Median:{' '}
                                <span className="font-mono font-medium">
                                  {analyticsData.operations.fileGeneration.linesPerSecond.median.toFixed(1)} lines/s
                                </span>
                              </div>
                              <div>
                                Peak (P99):{' '}
                                <span className="font-mono font-medium">
                                  {analyticsData.operations.fileGeneration.linesPerSecond.p99.toFixed(1)} lines/s
                                </span>
                              </div>
                            </div>
                          </div>
                          <div className="bg-bg-4/70 dark:bg-bg-4/50 rounded-lg p-4">
                            ...
                            <div className="space-y-1 text-sm">
                              <div>
                                Avg:{' '}
                                <span className="font-mono font-medium">
                                  {analyticsData.operations.fileGeneration.duration.avg > 1000
                                    ? `${(analyticsData.operations.fileGeneration.duration.avg / 1000).toFixed(1)}s`
                                    : `${analyticsData.operations.fileGeneration.duration.avg.toFixed(0)}ms`}
                                </span>
                              </div>
                              <div>
                                Median:{' '}
                                <span className="font-mono font-medium">
                                  {analyticsData.operations.fileGeneration.duration.median > 1000
                                    ? `${(analyticsData.operations.fileGeneration.duration.median / 1000).toFixed(1)}s`
                                    : `${analyticsData.operations.fileGeneration.duration.median.toFixed(0)}ms`}
                                </span>
                              </div>
                              <div>
                                P99:{' '}
                                <span className="font-mono font-medium">
                                  {analyticsData.operations.fileGeneration.duration.p99 > 1000
                                    ? `${(analyticsData.operations.fileGeneration.duration.p99 / 1000).toFixed(1)}s`
                                    : `${analyticsData.operations.fileGeneration.duration.p99.toFixed(0)}ms`}
                                </span>
                              </div>
                            </div>
                          </div>
                          <div className="bg-bg-4/70 dark:bg-bg-4/50 rounded-lg p-4">
                            ...
                            <div className="space-y-1 text-sm">
                              <div>
                                Total Characters:{' '}
                                <span className="font-mono font-medium">
                                  {analyticsData.operations.fileGeneration.totalChars.toLocaleString()}
                                </span>
                              </div>
                              <div>
                                Avg File Size:{' '}
                                <span className="font-mono font-medium">
                                  {Math.round(
                                    analyticsData.operations.fileGeneration.totalChars /
                                      analyticsData.operations.fileGeneration.duration.count,
                                  ).toLocaleString()}{' '}
                                  chars
                                </span>
                              ...
                              <div>
                                ...
                                <span className="font-mono font-medium">
                                  {Math.round(
                                    analyticsData.operations.fileGeneration.totalLines /
                                      analyticsData.operations.fileGeneration.duration.count,
                                  )}
                                </span>
                              </div>
                      ...
                      {Object.entries(analyticsData.operations)
                        ...
                        .map(([operation, stats]) => {
                          ...
                          return (
                            <div
                              key={operation}
                              className={`bg-${operationConfig.color}-50 border border-${operationConfig.color}-200 rounded-lg p-4`}
                            >
                                ...
                                <div className="space-y-3">
                                  <div className="text-center">
                                    <div className={`text-2xl font-bold text-${operationConfig.color}-600`}>
                                      {simpleStats.avg > 1000
                                        ? `${(simpleStats.avg / 1000).toFixed(1)}s`
                                        : `${simpleStats.avg.toFixed(0)}ms`}
                                    </div>
                                  ...
                                  <div className="grid grid-cols-2 gap-2 text-xs">
                                    <div>
                                      ...
                                      <div className="font-mono font-medium">
                                        {simpleStats.median > 1000
                                          ? `${(simpleStats.median / 1000).toFixed(1)}s`
                                          : `${simpleStats.median.toFixed(0)}ms`}
                                      </div>
                                    </div>
                                    <div>
                                      <div className="text-text-tertiary">P99</div>
                                      <div className="font-mono font-medium">
                                        {simpleStats.p99 > 1000
                                          ? `${(simpleStats.p99 / 1000).toFixed(1)}s`
                                          : `${simpleStats.p99.toFixed(0)}ms`}
                                      </div>
                                    </div>
                                  </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-sm text-text-tertiary">
                      Duration:{' '}
                      {(timelineData.events[timelineData.events.length - 1]?.timestamp || 0) -
                        (timelineData.events[0]?.timestamp || 0) >
                      1000
                      ...
                      {timelineData.events.map((event, index) => {
                        ...
                        return (
                          <div key={event.id} className="relative flex items-start">
                            {/* Timeline marker */}
                            <div
                              className={`relative z-10 flex items-center justify-center w-8 h-8 rounded-full border-2 ${lane.color} ${event.isBookmarked ? 'ring-2 ring-amber-400' : ''}`}
                            >
                              {event.type === 'error' ? (
                            ...
                            <div className="ml-4 flex-1 min-w-0">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  ...
                                  {event.isBookmarked && <Bookmark className="w-3 h-3 text-amber-500 fill-current" />}
                                </div>
                  ...
                  .map((message) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-sm text-text-primary mb-2">{message.message}</div>
