 ...
 */
export function updateFileInArray(files: FileType[], filePath: string, updates: Partial<FileType>): FileType[] {
  const existingFileIndex = files.findIndex((f) => f.filePath === filePath);

  if (existingFileIndex >= 0) {
    return files.map((file, index) => (index === existingFileIndex ? { ...file, ...updates } : file));
  }
 ...
 */
export function setFileGenerating(files: FileType[], filePath: string, explanation?: string): FileType[] {
 ...
 */
export function appendFileChunk(files: FileType[], filePath: string, chunk: string): FileType[] {
  const existingFile = files.find((f) => f.filePath === filePath);

  if (existingFile) {
    return files.map((file) =>
      file.filePath === filePath ? { ...file, fileContents: file.fileContents + chunk } : file,
    );
  }
 ...
 */
export function setFileCompleted(files: FileType[], filePath: string, fileContents: string): FileType[] {
  return updateFileInArray(files, filePath, {
    fileContents,
    isGenerating: false,
    needsFixing: false,
