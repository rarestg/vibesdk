...
export function updateStage(
  ...
  return stages.map((stage) => (stage.id === stageId ? { ...stage, ...updates } : stage));
}

/**
 * Complete multiple stages at once
 */
export function completeStages(stages: ProjectStage[], stageIds: ProjectStage['id'][]): ProjectStage[] {
  return stages.map((stage) => (stageIds.includes(stage.id) ? { ...stage, status: 'completed' as const } : stage));
}
 ...
 */
export function isStageCompleted(stages: ProjectStage[], stageId: ProjectStage['id']): boolean {
