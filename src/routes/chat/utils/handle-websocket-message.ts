  ...
  return (websocket: WebSocket, message: WebSocketMessage) => {
    ...
    switch (message.type) {
      case 'conversation_cleared': {
        ...
        setMessages(() =>
          appendToolEvent([], 'conversation_cleared', {
        ...
        if (!isInitialStateRestored) {
          ...
          if (templateDetails) {
            ...
            if (templateDetails.allFiles && bootstrapFiles.length === 0) {
              ...
              const files = Object.entries(templateDetails.allFiles)
      ...
      case 'conversation_state': {
        ...
        logger.debug(
          'Received conversation_state with messages:',
          history.length,
          'deepDebugSession:',
          deepDebugSession,
        );
        ...
        for (const msg of history) {
          ...
          if (msg.role === 'user') {
            ...
            const content = msg.conversationId.startsWith('archive-') ? 'previous history was compacted' : text || '';
        ...
        if (restoredMessages.length > 0) {
          ...
          logger.debug(
            'Merging conversation_state with',
            deduplicated.length,
            'messages (',
            restoredMessages.length - deduplicated.length,
            'duplicates removed)',
          );
          setMessages((prev) => {
            const hasFetching = prev.some((m) => m.role === 'assistant' && m.conversationId === 'fetching-chat');
            const hasReconnect = prev.some(
              (m) => m.role === 'assistant' && m.conversationId === 'websocket_reconnected',
            );
      ...
      case 'file_generated': {
        ...
        setPhaseTimeline((prev) =>
          updatePhaseFileStatus(prev, message.file.filePath, 'completed', message.file.fileContents),
        );
        deps.onPresentationFileEvent?.({
          type: 'file_generated',
          path: message.file.filePath,
          contents: message.file.fileContents,
        });
        break;
      }
      ...
      case 'code_reviewed': {
        ...
        const totalIssues =
          reviewData?.filesToFix?.reduce(
            (count: number, file: { issues: unknown[] }) => count + file.issues.length,
            0,
          ) || 0;
      ...
      case 'runtime_error_found': {
        ...
        onDebugMessage?.(
          ...
          message.errors
            .map((e: { message: string; stack?: string }) => `${e.message}\nStack: ${e.stack || 'N/A'}`)
            .join('\n\n'),
          'Runtime Detection',
        );
        break;
        ...
        if (message.phase) {
          setPhaseTimeline((prev) => {
            ...
            const newPhase = {
              ...
              files:
                message.phase.files?.map((f: { path: string; purpose: string }) => ({
                  path: f.path,
                  purpose: f.purpose,
      ...
      case 'cloudflare_deployment_completed': {
        ...
        sendMessage(
          createAIMessage(
            ...
            `Your project has been permanently deployed to Cloudflare Workers: ${message.deploymentUrl}`,
          ),
        );
      ...
      case 'cloudflare_deployment_error': {
        ...
        sendMessage(
          createAIMessage(
            ...
            `Deployment failed: ${message.error}\n\nYou can try deploying again.`,
          ),
        );
      ...
      case 'conversation_response': {
        ...
        if (message.conversationId) {
          ...
          setMessages((prev) => {
            const genericIdx = prev.findIndex(
              (m) => m.role === 'assistant' && m.conversationId === 'conversation_response',
            );
            if (genericIdx !== -1) {
        ...
        if (message.tool) {
          ...
          setMessages((prev) =>
            appendToolEvent(prev, conversationId, {
              name: tool.name,
              status: tool.status,
              result: tool.result,
            }),
        ...
        if (message.isStreaming) {
          setMessages((prev) =>
            handleStreamingMessage(prev, conversationId, isArchive ? placeholder : message.message, false),
          );
          break;
        }
        ...
        setMessages((prev) => {
          ...
          if (idx !== -1)
            return prev.map((m, i) => (i === idx ? { ...m, content: isArchive ? placeholder : message.message } : m));
      ...
      case 'error': {
        ...
        setMessages((prev) => [...prev, createAIMessage(`error_${Date.now()}`, `❌ ${errorData.error}`)]);

        onDebugMessage?.('error', 'WebSocket Error', errorData.error, 'WebSocket', 'error', errorData);
        break;
      }

      case 'rate_limit_error': {
        const errorData = message.error;
        const rateLimitMessage = handleRateLimitError(errorData.details, onDebugMessage);
