 ...
 */
export function createAIMessage(conversationId: string, content: string, isThinking?: boolean): ChatMessage {
  return {
    role: 'assistant',
    conversationId,
    content,
...
export function handleRateLimitError(
  ...
  return createAIMessage(`rate_limit_${Date.now()}`, `⏱️ ${displayMessage}`);
}

/**
 * Add or update a message in the messages array
 */
export function addOrUpdateMessage(messages: ChatMessage[], newMessage: ChatMessage): ChatMessage[] {
  ...
  if (newMessage.conversationId === 'main') {
    ...
    if (mainMessageIndex !== -1) {
      return messages.map((msg, index) => (index === mainMessageIndex ? { ...msg, ...newMessage } : msg));
    }
  }
...
export function handleStreamingMessage(
  ...
  if (existingMessageIndex !== -1 && !isNewMessage) {
    ...
    return messages.map((msg, index) =>
      index === existingMessageIndex ? { ...msg, content: msg.content + chunk } : msg,
    );
  } else {
