...
export function usePresentationFiles(allFiles: FileType[] = [], slideDirectory: string) {
  const slideFiles: SlideInfo[] = useMemo(() => {
    ...
    const manifestSlideNames = loadManifestSlides(`${slideDirectory}/manifest.json`).filter((name) =>
      name.endsWith('.json'),
    );
    const manifestSlidePaths = new Set(manifestSlideNames.map((name) => `${slideDirectory}/${name}`));
    ...
    const allSlideNames = [...manifestSlideNames, ...streamingSlideNames].filter(
