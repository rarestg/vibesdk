...
import { X, Github, Lock, Globe, Upload, CheckCircle, AlertCircle, Loader } from 'lucide-react';
...
const BUTTON_STYLES = {
  ...
  ghost:
    'bg-transparent hover:bg-bg-2 text-text-primary/60 hover:text-text-primary py-2 px-4 rounded-lg transition-colors',
...
const StatusIndicator: React.FC<{ type: 'success' | 'warning' | 'error'; message: string }> = ({ type, message }) => {
  const colors = {
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          <ModalHeader mode={mode} existingUrl={existingGithubUrl} onClose={handleClose} disabled={isExporting} />
          ...
          {showConflictWarning && remoteStatus && remoteStatus.aheadBy > 0 ? (
            <div className="py-6">
              ...
              <div className="text-sm text-text-primary/80 space-y-3">
                <p>
                  Remote has <strong>{remoteStatus.aheadBy} commit(s)</strong> not in your app. Your app has{' '}
                  <strong>{remoteStatus.behindBy} commit(s)</strong> not on GitHub.
                </p>
            ...
            exportResult.success ? (
              ...
              >
                <div className="space-x-2">
                  <button onClick={onRetry || (() => window.location.reload())} className={BUTTON_STYLES.secondary}>
                    Try Again
                  </button>
                  <button onClick={onClose} className={BUTTON_STYLES.ghost}>
                    Close
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="mb-4 p-3 bg-green-500/10 border border-green-500/20 rounded-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium text-green-600 dark:text-green-400">Last sync successful</p>
                      <a
              ...
              <div>
                <label className="block text-sm font-medium text-text-primary mb-2">Repository Name *</label>
                <input
                  type="text"
              ...
              {mode === 'sync' && existingGithubUrl && (
                <div className="p-3 bg-bg-2 rounded-lg space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-medium text-text-primary/60">Remote Repository</p>
                    <button
                      type="button"
                  ...
                  {isCheckingRemote ? (
                    ...
                    <div className="pt-1">
                      {remoteStatus.compatible ? (
                        ...
                        <StatusIndicator type="error" message="Incompatible histories - force push required" />
                      )}
                    </div>
                  ) : null}
                </div>
              ...
              {mode !== 'sync' && (
                <div>
                  <label className="block text-sm font-medium text-text-primary mb-3">Repository Privacy</label>
