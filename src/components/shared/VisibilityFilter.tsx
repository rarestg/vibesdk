...
export const VisibilityFilter: React.FC<VisibilityFilterProps> = ({ value, onChange, className }) => {
  return (
    <div className={cn('inline-flex items-center rounded-lg bg-bg-4 p-0.5', className)}>
      {visibilityOptions.map((option) => {
        ...
        return (
          ...
          >
            {isActive && (
              <motion.div
                className={cn('absolute inset-0 rounded-md bg-bg-1 shadow-sm')}
            ...
            <span className="relative flex items-center gap-1.5">
              <Icon className={cn('h-3 w-3 transition-colors duration-200', isActive && 'text-accent')} />
