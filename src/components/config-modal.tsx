...
const hasUserKeyForModel = (
  modelName: string,
  byokProviders: Array<{ provider: string; hasValidKey: boolean }>,
): boolean => {
...
export function ConfigModal({
  ...
  const selectedModelInfo = useMemo(() => {
    const currentModel = formData.modelName && formData.modelName !== 'default' ? formData.modelName : '';
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="overflow-y-auto max-w-3xl w-[90vw] max-h-[90vh] sm:max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>{agentConfig.description}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2 -mt-2">
          {agentConfig.constraint?.enabled && (
            <Alert>
              ...
              <AlertDescription className="text-sm">
                Model selection limited to {agentConfig.constraint.allowedModels.length} allowed model
                {agentConfig.constraint.allowedModels.length !== 1 ? 's' : ''} for this operation.
              </AlertDescription>
            </Alert>
          )}
          {getModelRecommendation(agentConfig.key) && (
            <Alert>
              <Info className="h-4 w-4" />
              <AlertDescription className="text-sm">{getModelRecommendation(agentConfig.key)}</AlertDescription>
            </Alert>
          )}
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between p-3 bg-bg-3/50 rounded-lg">
            ...
            <Badge variant={isUserOverride ? 'default' : 'outline'}>{isUserOverride ? 'Custom' : 'Default'}</Badge>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({ ...formData, reasoningEffort: value })}
                >
        ...
        <DialogFooter className="gap-3 flex-col sm:flex-row sm:justify-between">
          <div className="flex gap-2 flex-wrap">
            ...
            {isUserOverride && (
              <Button variant="ghost" size="sm" onClick={handleReset} className="gap-2 text-text-tertiary">
