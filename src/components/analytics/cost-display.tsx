...
export function CostDisplay({ cost, loading = false, variant = 'inline', className, label }: CostDisplayProps) {
  if (loading) {
    return (
      <div
        className={cn(
          'animate-pulse',
          variant === 'inline' ? 'h-4 w-12 bg-muted rounded' : 'h-20 w-48 bg-muted rounded-lg',
          className,
        )}
      />
    );
  ...
  if (variant === 'card') {
    return (
      <div className={cn('rounded-lg border bg-card p-4 space-y-2', className)}>
        ...
        <div className="text-2xl font-bold tabular-nums">{formatCost(cost)}</div>
      </div>
    );
  }
