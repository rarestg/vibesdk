...
function Card({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="card"
      className={cn('bg-bg-4 dark:bg-bg-2 text-text-primary flex flex-col rounded-md border', className)}
      {...props}
    />
  );
}
...
function CardHeader({ className, variant, ...props }: React.ComponentProps<'div'> & { variant?: 'minimal' }) {
  return (
    <div
      ...
      className={cn('flex items-start gap-1.5 py-4 px-4', className, variant !== 'minimal' ? 'py-4' : 'py-1')}
      {...props}
    />
  );
}
