...
export function useImageUpload(options: UseImageUploadOptions = {}): UseImageUploadReturn {
  const { maxImages = MAX_IMAGES_PER_MESSAGE, maxSizeBytes = MAX_IMAGE_SIZE_BYTES, onError } = options;
  ...
  const addImages = useCallback(
    async (files: File[]) => {
      ...
      try {
        ...
        // Process all files
        const processedImages = await Promise.all(files.map((file) => processImageFile(file)));
