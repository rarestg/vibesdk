...
export class AnalyticsService extends BaseService {
  ...
  async getUserStats(userId: string): Promise<UserStats> {
    ...
    const [appCount, publicAppCount, favoriteCount, totalLikesReceived, totalViewsReceived, streakDays] =
      await Promise.all([
        ...
        readDb
          .select({ count: count() })
          .from(schema.apps)
          .where(and(eq(schema.apps.userId, userId), eq(schema.apps.visibility, 'public')))
          .get()
          .then((r) => r?.count ?? 0),
