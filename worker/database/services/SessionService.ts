...
export class SessionService extends BaseService {
  ...
  async createSession(
    ...
    try {
      ...
      const { accessToken } = await this.jwtUtils.createAccessToken(userId, userEmail, sessionId);
      ...
      throw new SecurityError(SecurityErrorType.UNAUTHORIZED, 'Failed to create session', 500);
    }
  }
  ...
  async revokeUserSession(sessionId: string, userId: string): Promise<void> {
    try {
      ...
      throw new SecurityError(SecurityErrorType.UNAUTHORIZED, 'Failed to revoke session', 500);
    }
  }
  ...
  async revokeAllUserSessions(userId: string): Promise<void> {
    try {
      ...
      throw new SecurityError(SecurityErrorType.UNAUTHORIZED, 'Failed to revoke sessions', 500);
    }
  }
  ...
  async cleanupExpiredSessions(): Promise<number> {
    try {
      ...
      await this.db.db.delete(schema.sessions).where(lt(schema.sessions.expiresAt, now));
    ...
    try {
      ...
      if (sessions.length > SessionService.config.maxSessions) {
        ...
        for (const session of sessionsToDelete) {
          await this.db.db.delete(schema.sessions).where(eq(schema.sessions.id, session.id));
        }
  ...
  async forceLogoutAllOtherSessions(userId: string, currentSessionId: string): Promise<number> {
    try {
      ...
      // Log security event
      await this.logSecurityEvent(userId, currentSessionId, 'device_change', {
