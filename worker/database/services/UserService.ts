...
export class UserService extends BaseService {
  ...
  async findUser(options: {
    ...
    const whereConditions = [
      ...
      options.provider
        ? and(eq(schema.users.provider, options.provider.name), eq(schema.users.providerId, options.provider.id))
        : undefined,
  ...
  async findValidSession(sessionId: string): Promise<schema.Session | null> {
    const sessions = await this.database
      ...
      .where(and(eq(schema.sessions.id, sessionId), sql`${schema.sessions.expiresAt} > CURRENT_TIMESTAMP`))
      .limit(1);
  ...
  async cleanupExpiredSessions(): Promise<void> {
    const now = new Date();
    await this.database.delete(schema.sessions).where(lt(schema.sessions.expiresAt, now));
  }
  ...
  async isUsernameAvailable(username: string, excludeUserId?: string): Promise<boolean> {
    const existingUser = await this.database
      ...
      .from(schema.users)
      .where(and(eq(schema.users.username, username), excludeUserId ? ne(schema.users.id, excludeUserId) : undefined))
      .get();
  ...
  async getUserStatisticsBasic(userId: string): Promise<{ totalApps: number; appsThisMonth: number }> {
    ...
    const [totalApps, appsThisMonth] = await Promise.all([
      ...
      this.database
        .select({ count: sql<number>`COUNT(*)` })
        .from(schema.apps)
        .where(and(eq(schema.apps.userId, userId), sql`${schema.apps.createdAt} >= ${startOfMonth}`))
        .get()
        .then((r) => Number(r?.count) || 0),
    ]);
