 ...
 */
export function fixUndefinedName(context: FixerContext, issues: CodeIssue[]): FixResult {
  ...
  for (const [filePath, fileIssues] of issuesByFile) {
    try {
      const fileContent = getFileContent(filePath, context.files);
...
function analyzeUsageContext(fileContent: string, name: string, line: number): string {
  ...
  const typePattern = new RegExp(
    `(?::|extends|implements|satisfies)\\s+${name}\\b|\\b${name}\\s*<|as\\s+${name}\\b`,
    'g',
  );
  ...
  const arrayMethodPattern = new RegExp(
    `${name}\\s*\\.\\s*(map|filter|reduce|forEach|find|some|every|push|pop)\\s*\\(`,
    'g',
  );
...
function addDeclarationToAST(ast: t.File, declaration: string): t.File {
  try {
    ...
    const commentStatement = t.expressionStatement(t.identifier(`/* ${declaration} */`));
