 ...
 */
export function fixImportExportTypeMismatch(context: FixerContext, issues: CodeIssue[]): FixResult {
  ...
  for (const issue of issues) {
    ...
    try {
      ...
      logger.info(
        `Found import: ${importInfo.moduleSpecifier}, default: ${importInfo.defaultImport}, named: [${importInfo.namedImports.join(', ')}]`,
      );

      // Find the target file
      logger.info(`Searching for target file: ${importInfo.moduleSpecifier}`);
      const targetFile = findModuleFile(importInfo.moduleSpecifier, issue.filePath, context.files);
      ...
      logger.info(
        `Found exports - defaultExport: ${targetExports.defaultExport || 'none'}, named: [${targetExports.namedExports.join(', ')}]`,
      );
      ...
      logger.error(
        `Failed to fix TS2614 issue at ${issue.filePath}:${issue.line}: ${error instanceof Error ? error.message : 'Unknown error'}`,
        error,
      );
  ...
  logger.info(
    `TS2614 fixer completed: ${fixedIssues.length} fixed, ${unfixableIssues.length} unfixable, ${modifiedFiles.length} modified files, ${newFiles.length} new files`,
  );
...
function analyzeMismatch(
  importInfo: { defaultImport?: string; namedImports: string[] },
  ...
  if (importInfo.namedImports.length > 0 && !importInfo.defaultImport && targetExports.defaultExport) {
    ...
    const matchingNamedImport = importInfo.namedImports.find(
      (name) =>
        name === targetExports.defaultExport || name.toLowerCase() === targetExports.defaultExport?.toLowerCase(),
    );
  ...
  if (importInfo.defaultImport && !targetExports.defaultExport && targetExports.namedExports.length > 0) {
    ...
    const matchingNamedExport = targetExports.namedExports.find(
      (name) => name === importInfo.defaultImport || name.toLowerCase() === importInfo.defaultImport?.toLowerCase(),
    );
  ...
  if (importInfo.namedImports.length > 0) {
    const validNamedImports = importInfo.namedImports.filter((name) => targetExports.namedExports.includes(name));
    const invalidNamedImports = importInfo.namedImports.filter((name) => !targetExports.namedExports.includes(name));

    // Check if any invalid named imports match the default export
    const needsDefaultConversion = invalidNamedImports.filter(
      (name) =>
    ...
    const possibleTypos = invalidNamedImports
      .map((invalidName) => {
        ...
        switch (mismatchAnalysis.type) {
          case 'named-to-default':
            ...
            if (mismatchAnalysis.targetName) {
              ...
              const orig = path.node.specifiers.find(
                (s) =>
              ...
              path.node.specifiers = [t.importDefaultSpecifier(t.identifier(localName))];
            }
            break;
            ...
            if (mismatchAnalysis.additionalData) {
              ...
              if (defaultConversions.length > 0) {
                ...
                const defaultSpec = specifiers.find(
                  (s) => t.isImportSpecifier(s) && t.isIdentifier(s.imported) && s.imported.name === firstDefault,
                ) as t.ImportSpecifier | undefined;
                const defaultLocal =
                  defaultSpec && t.isIdentifier(defaultSpec.local) ? defaultSpec.local.name : firstDefault;
              ...
              if (mismatchAnalysis.sourceNames) {
                for (const validName of mismatchAnalysis.sourceNames) {
                  const orig = specifiers.find(
                    (s) => t.isImportSpecifier(s) && t.isIdentifier(s.imported) && s.imported.name === validName,
                  ) as t.ImportSpecifier | undefined;
              ...
              for (const correction of typoCorrections) {
                if (correction) {
                  const orig = specifiers.find(
                    (s) =>
                      t.isImportSpecifier(s) && t.isIdentifier(s.imported) && s.imported.name === correction.invalid,
                  ) as t.ImportSpecifier | undefined;
              ...
              const invalidSpec = specifiers.find(
                (s) =>
                  t.isImportSpecifier(s) &&
                  t.isIdentifier(s.imported) &&
                  s.imported.name === mismatchAnalysis.targetName,
              ...
              const defaultLocal =
                invalidSpec && t.isIdentifier(invalidSpec.local) ? invalidSpec.local.name : mismatchAnalysis.targetName;
              newSpecifiers.push(t.importDefaultSpecifier(t.identifier(defaultLocal)));
