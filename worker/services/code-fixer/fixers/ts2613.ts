 ...
 */
export function fixModuleIsNotModule(context: FixerContext, issues: CodeIssue[]): FixResult {
  ...
  for (const issue of issues) {
    ...
    try {
      ...
      logger.info(
        `Found import: ${importInfo ? importInfo.moduleSpecifier : namespaceImport?.moduleSpecifier || 'unknown'}, default: ${importInfo ? importInfo.defaultImport : 'none'}, named: [${importInfo ? importInfo.namedImports.join(', ') : 'none'}]`,
      );
      ...
      const targetFile = findModuleFile(moduleSpecifier, issue.filePath, context.files);
      ...
      logger.info(
        `Found exports - defaultExport: ${exports.defaultExport || 'none'}, named: [${exports.namedExports.join(', ')}]`,
      );
      ...
      if (namespaceImport) {
        ...
        const result = fixNamespaceImportMismatch(ast, namespaceImport, exports);
        fixed = result.fixed;
        changes = result.changes;
      } else if (importInfo) {
        // Regular import fix
        const result = fixImportExportMismatch(ast, moduleSpecifier, exports);
        fixed = result.fixed;
        changes = result.changes;
      }
  ...
  logger.info(
    `TS2613 fixer completed: ${fixedIssues.length} fixed, ${unfixableIssues.length} unfixable, ${modifiedFiles.length} modified files, ${newFiles.length} new files`,
  );
 ...
 */
function findNamespaceImportAtLocation(
  ast: t.File,
  line: number,
): { namespace: string; moduleSpecifier: string } | null {
      ...
      if (t.isStringLiteral(path.node.source) && path.node.source.value === namespaceImport.moduleSpecifier) {
        ...
        if (hasNamespace) {
          ...
          if (targetExports.defaultExport && targetExports.namedExports.length === 0) {
            ...
            path.node.specifiers = [t.importDefaultSpecifier(t.identifier(namespaceImport.namespace))];
