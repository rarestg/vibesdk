 ...
 */
export function fixIncorrectNamedImport(context: FixerContext, issues: CodeIssue[]): FixResult {
  ...
  for (const [filePath, fileIssues] of issuesByFile) {
    try {
      ...
      const sourceAST = getFileAST(filePath, context.files);
      ...
      for (const issue of fileIssues) {
        ...
        if (!importInfo.namedImports.includes(incorrectImport)) {
          logger.warn(
            `Incorrect import '${incorrectImport}' not found in named imports: ${importInfo.namedImports.join(', ')}`,
          );
          unfixableIssues.push(
            createUnfixableIssue(issue, `Named import '${incorrectImport}' not found in import statement`),
          );
          continue;
        }
        ...
        const targetFile = findModuleFile(resolvedPath, filePath, context.files);
        ...
        if (targetFile) {
          ...
          if (targetAST) {
            ...
            if (exports.namedExports.includes(suggestedImport) || exports.defaultExport === suggestedImport) {
              ...
              unfixableIssues.push(
                createUnfixableIssue(issue, `Suggested export '${suggestedImport}' not found in target module`),
              );
            }
          } else {
        ...
        if (fixedAST) {
          ...
          for (const issue of processedIssues) {
            ...
            if (parseResult) {
              ...
              logger.info(
                `Successfully fixed TS2724 issue: replaced '${parseResult.incorrectImport}' with '${parseResult.suggestedImport}' in ${issue.filePath}`,
              );
            }
          }
        }
