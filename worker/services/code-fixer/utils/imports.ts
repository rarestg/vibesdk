...
export function findImportAtLocation(ast: t.File, line: number): ImportInfo | null {
  ...
  if (foundImport) {
    ...
    logger.debug(
      `No import found at line ${line}. Available imports: ${allImports.map((i) => `${i.moduleSpecifier}:${i.line}`).join(', ')}`,
    );
  }

  logger.debug(
    `All imports found: count=${allImports.length}, first few: ${allImports
      .slice(0, 5)
      .map((i) => `${i.moduleSpecifier}:${i.line}`)
      .join(', ')}${allImports.length > 5 ? ', ...' : ''}`,
  );
  return foundImport;
}
...
export function analyzeNameUsage(ast: t.File, name: string): ImportUsage | null {
  ...
  traverseAST(ast, {
    ...
    JSXElement: (path) => {
      if (t.isJSXIdentifier(path.node.openingElement.name) && path.node.openingElement.name.name === name) {
    ...
    Identifier: (path) => {
      if (path.node.name === name && !path.isBindingIdentifier() && !usage) {
        // Only set as fallback if no specific usage found
 ...
 */
export function getFileContent(filePath: string, files: FileMap): string | null {
 ...
 */
export function getFileAST(filePath: string, files: FileMap): t.File | null {
        ...
        if (namespaceImport && t.isImportNamespaceSpecifier(namespaceImport)) {
          ...
          if (defaultImport) {
            ...
            if (exports.defaultExport) {
              ...
              const targetNamed =
                exports.namedExports.find((n) => n === localName || n.toLowerCase() === localName.toLowerCase()) ||
                exports.namedExports[0];

              if (targetNamed) {
                newSpecifiers.push(t.importSpecifier(t.identifier(localName), t.identifier(targetNamed)));
          ...
          for (const namedImport of namedImports) {
            if (t.isImportSpecifier(namedImport) && t.isIdentifier(namedImport.imported)) {
              ...
              if (exports.namedExports.includes(namedImportName)) {
                ...
                const caseInsensitiveMatch = exports.namedExports.find(
                  (n) => n.toLowerCase() === namedImportName.toLowerCase(),
                );
