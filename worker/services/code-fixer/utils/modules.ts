...
export function isExternalModule(moduleSpecifier: string): boolean {
  ...
  if (moduleSpecifier.includes('/') && (moduleSpecifier.includes('.') || moduleSpecifier.includes('src/'))) {
    return false;
  }
 ...
 */
export function resolveModuleFile(moduleSpecifier: string, fromFilePath: string, context: FixerContext): string | null {
  ...
  return findModuleFile(moduleSpecifier, fromFilePath, context.files);
}

/**
 * Check if a target file exists and can be modified
 */
export function canModifyTargetFile(targetFilePath: string, context: FixerContext): boolean {
  if (!canModifyFile(targetFilePath)) {
    return false;
  }

  try {
    const content = getFileContent(targetFilePath, context.files);
    return content !== null;
  } catch {
    return false;
  }
