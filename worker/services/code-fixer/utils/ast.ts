...
export function traverseAST(ast: t.Node, visitor: Visitor): void {
  try {
    ...
    const shouldRetryNoScope =
      message.includes("reading 'get'") || message.includes('Scope') || message.includes('setScope');
    if (shouldRetryNoScope) {
      logger.warn(`AST traversal failed with scope error, retrying with noScope: true`);
      try {
        (traverse as unknown as (node: t.Node, opts: Visitor & { noScope?: boolean }) => void)(ast, {
          ...(visitor as object),
          noScope: true,
        } as Visitor & { noScope?: boolean });
