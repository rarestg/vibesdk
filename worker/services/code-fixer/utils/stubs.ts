 ...
 */
export function analyzeImportUsageForStub(importInfo: ImportInfo, files: FileMap): ImportUsage[] {
  const sourceAST = getFileAST(importInfo.filePath, files);
  if (!sourceAST) return [];

  const importNames = [...(importInfo.defaultImport ? [importInfo.defaultImport] : []), ...importInfo.namedImports];
 ...
 */
export function generateStubFileAST(importInfo: ImportInfo, usages: ImportUsage[]): t.File {
  ...
  if (shouldUseJSX) {
    ...
    statements.push(t.importDeclaration([t.importDefaultSpecifier(t.identifier('React'))], t.stringLiteral('react')));
  }
 ...
 */
export function generateStubFileContent(importInfo: ImportInfo, files: FileMap): string {
...
function generateFunctionExport(usage: ImportUsage): t.Statement {
  ...
  const params = paramTypes.map((_, index) => t.identifier(`arg${index}`));

  const functionBody = t.blockStatement([t.returnStatement(t.nullLiteral())]);

  const functionDeclaration = t.functionDeclaration(t.identifier(functionName), params, functionBody);
...
function generateObjectExport(usage: ImportUsage): t.Statement {
  ...
  return t.exportNamedDeclaration(
    t.variableDeclaration('const', [t.variableDeclarator(t.identifier(objectName), objectExpression)]),
  );
}
...
function generateGenericExport(name: string, isDefault: boolean, shouldUseJSX: boolean): t.Statement {
  ...
  const declaration = t.variableDeclaration('const', [t.variableDeclarator(t.identifier(name), value)]);
