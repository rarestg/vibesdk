...
import { GitHubRepository, CreateRepositoryOptions, CreateRepositoryResult, GitHubServiceError } from './types';
...
export class GitHubService {
    ...
    try {
      ...
      if (octokitError?.status === 403) {
        return {
          success: false,
          error:
            'GitHub App lacks required permissions. Please ensure the app has Contents: Write and Metadata: Read permissions, then re-install it.',
        };
      }

      // Check if repository already exists (422 Unprocessable Entity)
      if (octokitError?.status === 422) {
        const hasNameExistsError = octokitError?.response?.data?.errors?.some(
          (e) => e.field === 'name' && e.message?.includes('already exists'),
        );
   ...
   */
  static async repositoryExists(options: { repositoryUrl: string; token: string }): Promise<boolean> {
  ...
  static async exportToGitHub(options: {
    ...
    try {
      ...
      const pushResult = await GitHubService.pushViaGitProtocol(fs, options.token, options.repositoryUrl);
      return pushResult;
    } catch (error) {
  ...
  private static async modifyReadmeForGitHub(fs: MemFS, githubRepoUrl: string): Promise<void> {
    try {
      ...
      const modified = content.replaceAll('[cloudflarebutton]', prepareCloudflareButton(githubRepoUrl, 'markdown'));
  ...
  static async checkRemoteStatus(options: {
    ...
    try {
      ...
      const localMessages = new Set(localCommits.map((c) => GitHubService.normalizeCommitMessage(c.commit.message)));
      const remoteMessages = new Set(remoteCommits.map((c) => GitHubService.normalizeCommitMessage(c.commit.message)));

      const localOnly = localCommits.filter(
        (c) => !remoteMessages.has(GitHubService.normalizeCommitMessage(c.commit.message)),
      );
