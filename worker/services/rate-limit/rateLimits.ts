...
export class RateLimitService {
  ...
  static async enforceAuthRateLimit(env: Env, config: RateLimitSettings, user: AuthUser | null, request: Request) {
  ...
  static async enforceAppCreationRateLimit(
    ...
    try {
      ...
      if (!result.success) {
        ...
        const periodLabel =
          result.exceededLimit === 'daily' ? 'day' : `${periodHours} hour${periodHours >= 2 ? 's' : ''}`;
