...
import { buildDeploymentConfig, parseWranglerConfig, deployToDispatch, deployWorker } from '../deployer/deploy';
...
import { isDev, isEnabledEnvFlag } from 'worker/utils/envs';
...
export class SandboxSdkClient extends BaseSandboxService {
        ...
        if (logsResult.success && logsResult.logs.stdout) {
          ...
          for (const pattern of readinessPatterns) {
            if (pattern.test(logs)) {
              ...
              this.logger.info('Development server ready', {
                instanceId,
  ...
  private async provisionTemplateResources(
    ...
    try {
      ...
      for (const placeholderInfo of parseResult.placeholders) {
        ...
        const provisionResult = await resourceProvisioner.provisionResource(placeholderInfo.resourceType, projectName);
  ...
  private async startCloudflaredTunnel(
    ...
    options?: {
      waitForUrl?: boolean;
      urlTimeoutMs?: number;
      onUrlDetected?: (url: string, processId: string) => void | Promise<void>;
    },
    ...
    try {
      ...
      if (!waitForUrl) {
        ...
        return { url: '', processId: process.id };
      }

      const url = await waitForUrlPromise;
      return { url, processId: process.id };
    } catch (error) {
  ...
  async createInstance(options: InstanceCreationRequest): Promise<BootstrapResponse> {
    ...
    try {
      ...
      if (envVars && Object.keys(envVars).length > 0) {
        this.logger.info('Environment variables will be configured via session', {
      ...
      if (env.ALLOCATION_STRATEGY === 'one_to_one') {
        ...
        if (instancesResp.success && instancesResp.instances.length > 0) {
          ...
          if (instanceStatus.success && (instanceStatus.isHealthy || instanceStatus.pending)) {
            ...
            this.logger.error('Instance already exists but is not active, Removing old instance', {
              instance: firstInstance,
            });
              ...
              try {
                ...
                const restartedTunnel = await this.startCloudflaredTunnel(instanceId, metadata.allocatedPort, {
                  ...
                  onUrlDetected: async (url, processId) => {
                    const latestMetadata = await this.getInstanceMetadata(instanceId);
                    if (!latestMetadata) {
                      return;
                    }
  ...
  async writeFiles(instanceId: string, files: WriteFilesRequest['files']): Promise<WriteFilesResponse> {
    try {
      ...
      if (wereDontTouchFiles.length > 0) {
        this.logger.warn('Files were not written (protected by donttouch_files)', {
      ...
      if (
        successCount > 0 &&
        filteredFiles.some((file) => file.filePath.endsWith('.ts') || file.filePath.endsWith('.tsx'))
      ) {
  ...
  async deployToCloudflareWorkers(
    ...
    try {
      ...
      if (useDispatch) {
        ...
        await deployWorker(deployConfig, fileContents, additionalModules, config.migrations, config.assets);
      }
