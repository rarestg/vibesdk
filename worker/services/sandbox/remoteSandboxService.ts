...
export class RemoteSandboxServiceClient extends BaseSandboxService {
  ...
  private async makeRequest<T extends z.ZodTypeAny>(
    ...
    try {
      ...
      if (!validation.success) {
        this.logger.error('Failed to validate response from runner service', validation.error.errors, {
          url,
          responseData,
        });
        return {
  ...
  async getFiles(instanceId: string, filePaths?: string[]): Promise<GetFilesResponse> {
    ...
    const queryParams =
      filePaths && filePaths.length > 0 ? `?filePaths=${encodeURIComponent(JSON.stringify(filePaths))}` : '';
  ...
  async pushToGitHub(
    ...
    return this.makeRequest(`/instances/${instanceId}/github/push`, 'POST', GitHubPushResponseSchema, {
      request,
      files,
    });
  }
