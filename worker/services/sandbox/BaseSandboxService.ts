...
export abstract class BaseSandboxService {
  ...
  static async getTemplateDetails(templateName: string, downloadDir?: string): Promise<TemplateDetailsResponse> {
    try {
      ...
      const filteredFiles = allFiles.filter(
        (f) => !f.filePath.startsWith('.') || (!f.filePath.endsWith('.json') && !f.filePath.startsWith('.git')),
      );
   ...
   */
  abstract createInstance(options: InstanceCreationRequest): Promise<BootstrapResponse>;
