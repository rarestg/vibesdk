...
export function getSystemPromptWithProjectContext(
  ...
  const messages = [
    ...
    createAssistantMessage(
      `I have thoroughly gone through the whole codebase and understood the current implementation and project requirements. We can continue.`,
    ),
  ];
  return messages;
}
...
export abstract class AgentOperation<
  ...
  TOutput = unknown,
> {
  abstract execute(inputs: TInput, options: OperationOptions<TContext>): Promise<TOutput>;
}
...
> extends AgentOperation<TContext, TInput, TOutput> {
  ...
  protected abstract getCallbacks(inputs: TInput, options: OperationOptions<TContext>): ToolCallbacks;

  protected abstract buildSession(inputs: TInput, options: OperationOptions<TContext>): TSession;
  ...
  protected createCompletionConfig(
    ...
    return {
      detector: completionSignalName ? new CompletionDetector([completionSignalName]) : undefined,
      operationalMode,
  ...
  protected async runToolInference(
    ...
    const { messages, tools, agentActionName, streamCb, onAssistantMessage, completionConfig } = params;
      ...
      ? (chunk: string) => {
          ...
          if (accumulatedContent.length - lastCheckLength > CHECK_INTERVAL) {
            ...
            if (this.loopDetector.detectTextRepetition(recentContent)) {
              ...
              throw new InferError('Text repetition detected', accumulatedContent + '\n\n' + warning);
            }
          }
        }
      : undefined;
  ...
  async execute(inputs: TInput, options: OperationOptions<TContext>): Promise<TOutput> {
    ...
    const { agentActionName, completionSignalName, operationalMode, allowWarningInjection } = this.getAgentConfig(
      inputs,
      options,
      session,
    );
    ...
    const completionConfig = completionSignalName
      ? this.createCompletionConfig(completionSignalName, operationalMode ?? 'initial', allowWarningInjection ?? false)
      : undefined;
