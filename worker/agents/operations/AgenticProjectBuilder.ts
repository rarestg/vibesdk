import { createSystemMessage, createUserMessage, Message, ConversationMessage } from '../inferutils/common';
 ...
 */
const getUserPrompt = (query: string, projectName: string, fileSummaries: string, templateInfo?: string): string => {
  return `## Build Task
...
> {
  ...
  protected buildSession(
    ...
    const { projectName, projectType, blueprint, filesIndex, selectedTemplate } = inputs;
  ...
  protected async buildMessages(
    ...
    const userPrompt = getUserPrompt(inputs.query, inputs.projectName, session.fileSummaries, session.templateInfo);
