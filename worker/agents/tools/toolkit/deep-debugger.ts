  ...
  return tool({
    ...
    run: async ({ issue, focus_paths }) => {
      if (callCount > 0) {
        ...
        return {
          error:
            'CALL_LIMIT_EXCEEDED: You are only allowed to make a single deep_debug call per conversation turn. Ask user for permission before trying again.',
        };
      }
      ...
      if (agent.isCodeGenerating()) {
        ...
        return {
          error:
            'GENERATION_IN_PROGRESS: Code generation is currently running. Use wait_for_generation tool, then retry deep_debug.',
        };
      }
      ...
      if (agent.isDeepDebugging()) {
        ...
        return {
          error:
            "DEBUG_IN_PROGRESS: Another debug session is currently running. Wait for it to finish, and if it doesn't, solve the issue, Use wait_for_debug tool, then retry deep_debug.",
        };
      }
