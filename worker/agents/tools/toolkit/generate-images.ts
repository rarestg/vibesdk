...
export function createGenerateImagesTool(_agent: ICodingAgent, _logger: StructuredLogger) {
  return tool({
    ...
    run: async ({ prompts, style }) => {
      return {
        message: `Image generation not implemented yet. Requested ${prompts.length} prompt(s)${style ? ` with style ${style}` : ''}.`,
      };
    },
  });
}
