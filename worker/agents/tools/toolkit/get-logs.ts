...
export function createGetLogsTool(agent: ICodingAgent, logger: StructuredLogger) {
  return tool({
    name: 'get_logs',
    description: `Get cumulative application/server logs from the sandbox environment.
...
**RESET:** Set reset=true to clear accumulated logs before fetching. Use when starting fresh debugging or after major fixes.`,
    args: {
      reset: t.logs
        .reset()
        .describe(
          'Clear accumulated logs before fetching. Default: false. Set to true when starting fresh debugging or after major fixes to avoid stale errors.',
        ),
      durationSeconds: t.logs
        .durationSeconds()
        .describe(
          'Time window in seconds. Default: 30 seconds (recent activity). Set to higher value if you need older logs.',
        ),
      maxLines: t.logs
        .maxLines()
        .describe(
          'Maximum lines to return. Default: 100. Set to -1 for no truncation (warning: heavy token usage). Increase to 200-500 for more context.',
        ),
    },
