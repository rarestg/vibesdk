...
export function createVirtualFilesystemTool(agent: ICodingAgent, logger: StructuredLogger) {
  return tool({
...
IMPORTANT: This reads from the VIRTUAL filesystem, NOT the sandbox. Files appear here immediately after generation and may not be deployed to sandbox yet.`,
    args: {
      command: t
        ...
        .describe('Action to perform: "list" shows all files, "read" returns file contents, "delete" deletes files'),
      paths: t.files
        .read()
        .optional()
        .describe(
          'File paths to read/delete (required when command="read" or "delete"). Use relative paths from project root.',
        ),
    },
