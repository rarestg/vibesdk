...
const submitFeedbackImplementation = async (args: FeedbackArgs): Promise<FeedbackResult> => {
  try {
    ...
    const eventId = withScope((scope) => {
      ...
      return captureMessage(args.message, args.type === 'bug' ? 'error' : 'info');
    });
    ...
    return {
      error: error instanceof Error ? `Failed to submit: ${error.message}` : 'Unknown error occurred',
    };
  }
};
...
export const toolFeedbackDefinition = tool({
  ...
  args: {
    ...
    type: t
      .enum(['bug', 'feedback'] as const)
      .describe("'bug' for persistent technical issues, 'feedback' for feature requests or general comments"),
    severity: t
      .enum(['low', 'medium', 'high'] as const)
      .optional()
      .describe("Severity level - 'high' only for critical blocking issues"),
    context: t
      .string()
      .optional()
      .describe('Additional context about the project, what the user was trying to build, or environment details'),
  },
