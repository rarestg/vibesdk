...
export class FileProcessing {
  ...
  static getAllFiles(
    ...
    const templateFiles = templateDetails?.allFiles
      ? onlyImportantFiles
        ? getTemplateImportantFiles(templateDetails)
        : getTemplateFiles(templateDetails)
      : [];

    // Filter out template files that have been overridden by generated files
    const nonOverriddenTemplateFiles = templateFiles.filter((file) => !generatedFilesMap[file.filePath]);
