...
`;
  ...
  private handleClosingTag(tagName: string, state: XmlParsingState, callbacks: XmlStreamingCallbacks): boolean {
    ...
    if (state.currentElement.tagName.toLowerCase() !== normalizedTagName) {
      this.handleParsingError(
        state,
        `Mismatched closing tag: expected ${state.currentElement.tagName}, got ${tagName}`,
        callbacks,
      );
      return false;
    }
  ...
  private addContentToCurrentElement(state: XmlParsingState, content: string, callbacks: XmlStreamingCallbacks): void {
  ...
  private handleParsingError(state: XmlParsingState, error: string, callbacks: XmlStreamingCallbacks): void {
