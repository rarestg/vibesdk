...
export class SCOFFormat extends CodeGenerationFormat {
  ...
  private tryParseCommand(
    commandStr: string,
  ): { type: 'file_creation' | 'diff_patch'; filePath: string; eofMarker: string } | null {
    ...
    for (const pattern of unquotedPatchPatterns) {
      ...
      if (match) {
        ...
        if (!eofMarker.match(/^['"]/) && !filePath.match(/^['"]/)) {
          console.warn(
            `SCOF: Detected potentially malformed patch command. Auto-correcting: EOF="${eofMarker}", file="${filePath}"`,
          );
        }
