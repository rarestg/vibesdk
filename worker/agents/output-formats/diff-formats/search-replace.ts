...
function parseSearchReplaceDiff(diffContent: string): ParseResult {
    ...
    switch (state) {
      ...
      case ParserState.IN_SEARCH:
        if (line === '<<<<<<< SEARCH') {
          ...
          if (currentBlock) {
            errors.push(
              `Block at line ${currentBlock.startLine + 1}: SEARCH block without corresponding REPLACE (followed by another SEARCH)`,
            );
          }
          // Start new search block
          currentBlock = {
      ...
      case ParserState.IN_REPLACE:
        ...
        if (line === '>>>>>>> REPLACE') {
          ...
          errors.push(
            `Block at line ${currentBlock.startLine + 1}: REPLACE block ended prematurely (no end marker found)`,
          );
          completeBlock(currentBlock, blocks, errors);
          ...
          errors.push(
            `Block at line ${currentBlock.startLine + 1}: Malformed block with multiple separators - block ignored`,
          );
  ...
  if (currentBlock) {
    switch (state) {
      case ParserState.IN_SEARCH:
        errors.push(
          `Block at line ${currentBlock.startLine + 1}: SEARCH block without corresponding REPLACE (end of file reached)`,
        );
        break;
      case ParserState.IN_REPLACE:
        errors.push(
          `Block at line ${currentBlock.startLine + 1}: REPLACE block ended prematurely (end of file reached)`,
        );
        break;
    }
  }
...
function extractIndentationPattern(text: string): { pattern: string; lines: string[] } {
  ...
  const pattern = indentedLines
    .map(({ indent, content }) => (content.trim() ? `${indent.length}:${content.trim()}` : ''))
    .join('\n');
...
function findActualMatchBoundaries(
  content: string,
  searchText: string,
...
function findPatternMatchBoundaries(
  content: string,
  searchText: string,
  _contentPattern: { pattern: string; lines: string[] },
  searchPattern: { pattern: string; lines: string[] },
 ...
 */
function analyzeSearchBlockQuality(
  searchText: string,
    ...
    case MatchingStrategy.FUZZY: {
      ...
      for (let i = 0; i <= lines.length - searchLines.length; i++) {
        ...
        if (similarity >= adjustedThreshold) {
          ...
          const contextScore = validateMatchContext(content, candidate, i, i + searchLines.length);
          if (contextScore > 0.6) {
            // Only count matches with good context
            count++;
          }
        }
      }
...
export function applyDiff(
  ...
  const defaultOptions = {
    ...
    fuzzyThreshold: 0.8, // 85% similarity threshold - good balance of flexibility and precision
  };
  ...
  try {
    ...
    for (const block of blocks) {
      const {
        ...
        error,
        strategy,
      } = applySearchReplaceBlock(currentContent, block, {
    ...
    if (result.results.blocksApplied === 0 && result.results.blocksTotal > 0) {
      const errorDetails = failedBlocks
        .map(({ block, error }) => {
 ...
 */
export function validateDiff(content: string, diffContent: string): { valid: boolean; errors: string[] } {
  const { blocks, errors: parseErrors } = parseSearchReplaceDiff(diffContent);
