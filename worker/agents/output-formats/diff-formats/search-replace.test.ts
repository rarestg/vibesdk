...
>>>>>>> REPLACE`;
      ...
      expect(() => applyDiff(original, diff, { strict: true })).toThrow('found 2 times (ambiguous)');
    });
...
const x = 2;`; // Missing end marker
      ...
      expect(() => applyDiff(original, diff, { strict: true })).toThrow('REPLACE block ended prematurely');
    });
...
>>>>>>> REPLACE`;
      ...
      expect(() => applyDiff(original, diff, { strict: true })).toThrow('SEARCH block without corresponding REPLACE');
    });
...
>>>>>>> REPLACE`;
      ...
      expect(() => applyDiff(original, diff, { strict: true })).toThrow('Search block not found');
    });
...
>>>>>>> REPLACE`;
      ...
      expect(result.results.errors.some((error) => error.includes('Malformed block with multiple separators'))).toBe(
        true,
      );
