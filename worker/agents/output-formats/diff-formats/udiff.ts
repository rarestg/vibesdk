...
function normalizeIndentation(lines: string[]): {
  ...
  for (const line of nonEmptyLines.slice(1)) {
    ...
    let i = 0;
    while (i < Math.min(commonPrefix.length, prefix.length) && commonPrefix[i] === prefix[i]) {
      i++;
    }
...
function breakIntoSubHunks(hunk: string[], maxContextLines: number = 3): string[][] {
  ...
  for (const run of changeRuns) {
    ...
    const contextBefore = hunk
      .slice(Math.max(0, run.start - maxContextLines), run.start)
      .filter((line) => line.startsWith(' ')); // Only context lines

    const contextAfter = hunk
      .slice(run.end + 1, Math.min(hunk.length, run.end + 1 + maxContextLines))
      .filter((line) => line.startsWith(' ')); // Only context lines
...
function tryWithWhitespaceNormalization(content: string, before: string[], after: string[]): string | null {
  ...
  for (let i = 0; i <= contentLines.length - before.length; i++) {
    ...
    if (matches) {
      ...
      const resultLines = [...originalLines.slice(0, i), ...processedAfter, ...originalLines.slice(i + before.length)];
  ...
  for (let i = 0; i <= contentLines.length - before.length; i++) {
    ...
    if (matches) {
      ...
      const indentedAfter = after.map((afterLine, idx) => {
        ...
        const originalIndent = indentMap.get(correspondingBeforeLine) || contentSlice[idx]?.match(/^[\t ]*/)?.[0] || '';

        return originalIndent + trimmedAfter;
      });

      const newLines = [...contentLines.slice(0, i), ...indentedAfter, ...contentLines.slice(i + before.length)];
...
function tryWithNormalizedWhitespace(content: string, hunk: string[]): string | null {
  ...
  const replacementLines = normalizedAfter.map((line) => (line.length > 0 ? actualIndentation + line : line));
...
function tryContextReduction(content: string, hunk: string[]): string | null {
  // Find core changes
  const firstChangeIndex = hunk.findIndex((l) => l.startsWith('-') || l.startsWith('+'));
  const lastChangeIndex =
    hunk.length - 1 - [...hunk].reverse().findIndex((l) => l.startsWith('-') || l.startsWith('+'));
  ...
  for (let i = precedingContext.length; i >= 0; i--) {
    for (let j = followingContext.length; j >= 0; j--) {
      ...
      const searchBefore = hunkToBeforeAfter([...contextBefore, ...coreChangeHunk, ...contextAfter]).before;
      ...
      if (searchOccurrences === 1) {
        const replaceAfter = hunkToBeforeAfter([...contextBefore, ...coreChangeHunk, ...contextAfter]).after;
...
class DiffSecurityValidator {
  static validateContent(content: string): void {
    ...
    if (content.length > PRODUCTION_LIMITS.MAX_CONTENT_SIZE) {
      throw new Error(
        `Content too large: ${content.length} bytes exceeds ${PRODUCTION_LIMITS.MAX_CONTENT_SIZE} byte limit`,
      );
    }
  }
...
class PerformanceMonitor {
  ...
  getTelemetry(): Omit<
    DiffTelemetry,
    'strategiesAttempted' | 'contentSize' | 'diffSize' | 'hunkCount' | 'success' | 'errorDetails'
  > {
    return {
...
export function applyDiff(
  ...
  try {
    ...
    const hunksRaw = cleanedDiff.match(/(?:^|\n)@@[^\n]*(?:\n(?!@@)[^\n]*)*(?=\n@@|$)/g) || [cleanedDiff]; // Fallback to single hunk if no matches
    ...
    const hunks = hunksRaw.map((h, idx) => {
      ...
      if (hunkLines.length > PRODUCTION_LIMITS.MAX_HUNK_SIZE) {
        throw new Error(
          `Hunk #${idx + 1} too large: ${hunkLines.length} lines exceeds ${PRODUCTION_LIMITS.MAX_HUNK_SIZE} limit`,
        );
      }
