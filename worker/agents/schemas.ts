...
export const TemplateSelectionSchema = z.object({
  selectedTemplateName: z
    .string()
    .nullable()
    .describe('The name of the most suitable template, or null if none are suitable.'),
  reasoning: z.string().describe('Brief explanation for the selection or why no template was chosen.'),
  useCase: z
    .enum(['SaaS Product Website', 'Dashboard', 'Blog', 'Portfolio', 'E-Commerce', 'General', 'Other'])
    .describe('The use case for which the template is selected, if applicable.')
    .nullable(),
  complexity: z
    .enum(['simple', 'moderate', 'complex'])
    .describe('The complexity of developing the project based on the the user query')
    .nullable(),
  styleSelection: z
    .enum(['Minimalist Design', 'Brutalism', 'Retro', 'Illustrative', 'Kid_Playful', 'Custom'])
...
export const FileGenerationOutput = FileOutputSchema.extend({
  format: z
    ...
    .describe('`full_content` for full, raw file contents, `unified_diff` for unified diff'),
});
...
export const CodeReviewOutput = z.object({
  ...
  filesToFix: z
    .array(
      z.object({
        ...
        issues: z
          .array(z.string())
          .describe('List of issues found in this file and actionable recommendations for fixing them'),
  ...
  commands: z
    .array(z.string())
    .describe('Commands that might be needed to run for fixing an issue. Empty array if no commands are needed'),
});
...
export const SimpleBlueprintSchema = z.object({
  ...
  colorPalette: z
    .array(z.string())
    .describe('Color palette RGB codes to be used in the project, only base colors and not their shades, max 3 colors'),
  frameworks: z
    .array(z.string())
    .describe(
