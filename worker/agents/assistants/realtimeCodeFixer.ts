  ...
  async run(
    ...
    try {
      ...
      while (searchBlocks !== 0 && i < passes) {
        ...
        this.logger.info(
          `Running realtime code fixer for file: ${generatedFile.filePath} (pass ${i + 1}/${passes}), issues: ${JSON.stringify(issues, null, 2)}`,
        );
...
`);
      ...
      this.logger.info(
        `Realtime code fixer completed for file: ${generatedFile.filePath} in ${endTime - startTime}ms, found ${searchBlocks} search blocks`,
      );
    ...
    try {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        if (searchBlocks !== replaceBlocks) {
          this.logger.warn(
            `Realtime code fixer returned mismatched search and replace blocks for file, ${searchBlocks} search blocks and ${replaceBlocks} replace blocks`,
          );
        ...
        this.logger.info(
          `${attempt === 0 ? 'Initial' : `Retry ${attempt}`} application: ${blocksApplied}/${blocksTotal} blocks applied`,
        );
...
-------------------------`);
  ...
  async getLLMCorrectedDiff(
    ...
    try {
      ...
      const failedBlocksText = failedBlocks
        .map(
          (block) =>
...
export function IsRealtimeCodeFixerEnabled(inferenceContext: InferenceContext): boolean {
  ...
  if (
    ...
    inferenceContext.userModelConfigs['realtimeCodeFixer'].name !== AIModels.DISABLED
  ) {
