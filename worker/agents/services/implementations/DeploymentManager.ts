...
export class DeploymentManager extends BaseAgentService<BaseProjectState> implements IDeploymentManager {
  ...
  public getClient(): BaseSandboxService {
    if (!this.cachedSandboxClient) {
      ...
      this.cachedSandboxClient = getSandboxService(this.getSessionId(), this.getAgentId());
    }
  ...
  async executeSetupCommands(
    ...
    logger.info(
      `[commands] Executing ${validCommands.length} validated setup commands on instance ${sandboxInstanceId}`,
    );
  ...
  async runStaticAnalysis(files?: string[]): Promise<StaticAnalysisResponse> {
    ...
    const targetFiles = Array.isArray(files) && files.length > 0 ? files : this.fileManager.getGeneratedFilePaths();

    const analysisResponse = await client.runStaticAnalysisCode(sandboxInstanceId, targetFiles);
    ...
    logger.info(
      `Type checking found ${typeCheckIssues.length} issues: ` +
  ...
  private async executeDeploymentWithRetry(
    ...
    while (true) {
      ...
      if (this.deploymentGeneration !== myGeneration) {
        ...
        throw new Error('Deployment loop superseded by newer deployment');
      }
      ...
      try {
        ...
        const result = await this.withTimeout(
          ...
          () => {
            ...
            logger.warn(`Deployment attempt ${attempt} exceeded timeout; resetting sessionId to isolate retry`);
        ...
        if (!sessionResetForAttempt && attempt % maxAttemptsBeforeSessionReset === 0) {
  ...
  private async deploy(params: DeploymentParams): Promise<DeploymentResult> {
    ...
    if (filesToWrite.length > 0) {
      const writeResponse = await this.getClient().writeFiles(sandboxInstanceId, filesToWrite, commitMessage);
      ...
      logger.info('Files written to sandbox instance', {
        ...
        files: filesToWrite.map((f) => f.filePath),
      });
    }
    ...
    if (state.templateName?.includes('agents')) {
      ...
      if (typeof secret === 'string' && secret.trim().length > 0) {
        localEnvVars = {
          ...
          CF_AI_API_KEY: await generateAppProxyToken(state.metadata.agentId, state.metadata.userId, this.env),
        };
      }
    }
  ...
  async deployToCloudflare(request?: {
    ...
    const deploymentResult = await client.deployToCloudflareWorkers(state.sandboxInstanceId, target);
