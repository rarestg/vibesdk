...
import type { Root, Content, Node as UnistNode, Heading, List, ListItem, Code, ThematicBreak } from 'mdast';
...
export function formatSchemaAsMarkdown<T extends z.ZodRawShape>(
...
function formatZodSchemaAsMarkdownFields(
  ...
  for (const key of Object.keys(shape)) {
    ...
    if (dataObject) {
      ...
      while (
        checkSchema instanceof z.ZodOptional ||
        checkSchema instanceof z.ZodNullable ||
        checkSchema instanceof z.ZodDefault
      ) {
...
function getMarkdownPlaceholderValue(key: string, field: z.ZodTypeAny, headingPrefix: string): string {
  ...
  if (field instanceof z.ZodArray) {
    ...
    value = field._def.checks?.find((c) => c.kind === 'uuid')
      ? '[UUID string]'
      : field._def.checks?.find((c) => c.kind === 'email')
        ? '[email string]'
        : field._def.checks?.find((c) => c.kind === 'url')
...
function formatZodTypeAsMarkdown(
  ...
  while (
    baseField instanceof z.ZodOptional ||
    baseField instanceof z.ZodNullable ||
    baseField instanceof z.ZodDefault
  ) {
  ...
  const description =
    mode === 'template' && !hasValue && field.description ? `\n\n*Description: ${field.description}*` : '';
      ...
      value.forEach((item, index) => {
        ...
        if (itemSchema instanceof z.ZodObject) {
          if (item && typeof item === 'object') {
            ...
            if (mode === 'template') {
              logger.warn(
                `[Format Data Path: ...${key}[${index}]] Expected object item but got ${typeof item}. Rendering empty item.`,
              );
...
function convertToPrimitive(
  value: any,
  schema: z.ZodTypeAny,
  debugInfo?: { path: string | string[]; logger: any; debug: boolean },
): any {
  ...
  if (typeof value !== 'string') {
    if (debug)
      currentLogger.debug(
        `[Convert Path: ${pathStr}] Input is not a string (${typeof value}). Value: ${JSON.stringify(value)}`,
      );
    ...
    if ((value === null || value === undefined) && schema instanceof z.ZodDefault) {
      if (debug)
        currentLogger.debug(`[Convert Path: ${pathStr}] Non-string null/undefined for ZodDefault. Returning default.`);
      return getDefaultValue(schema);
    }
    // Handle null for ZodNullable when input is undefined
    if (value === undefined && schema instanceof z.ZodNullable) {
      if (debug)
        currentLogger.debug(`[Convert Path: ${pathStr}] Non-string undefined for ZodNullable. Returning null.`);
      return null;
    }
    ...
    if (debug)
      currentLogger.debug(
        `[Convert Path: ${pathStr}] Non-string type (${typeof value}) doesn't match schema ${baseSchema.constructor.name}. Converting to string.`,
      );
  ...
  if (debug)
    currentLogger.debug(
      `[Convert Path: ${pathStr}] Processing trimmed string value: "${stringValue.substring(0, 100)}..."`,
    );
  ...
  if (schema instanceof z.ZodOptional || schema instanceof z.ZodNullable || schema instanceof z.ZodDefault) {
    ...
    const isNullish =
      isEmpty ||
      ['null', 'none', 'n/a', 'undefined', 'nil', 'empty', 'missing', '[no value]', '[not applicable]'].includes(
        stringValue.toLowerCase(),
      );

    if (debug)
      currentLogger.debug(
        `[Convert Path: ${pathStr}] Checking modifiers: Optional=${schema instanceof z.ZodOptional}, Nullable=${schema instanceof z.ZodNullable}, Default=${schema instanceof z.ZodDefault}. IsEmpty=${isEmpty}, IsNullish=${isNullish}`,
      );
    ...
    if (isNullish) {
      if (schema instanceof z.ZodDefault) {
        if (debug)
          currentLogger.debug(
            `[Convert Path: ${pathStr}] Empty/nullish string for ZodDefault. Returning default value.`,
          );
        return getDefaultValue(schema); // Return default *before* attempting inner parse
      }
      if (schema instanceof z.ZodOptional) {
        if (debug)
          currentLogger.debug(`[Convert Path: ${pathStr}] Empty/nullish string for ZodOptional. Returning undefined.`);
        return undefined;
      }
      if (schema instanceof z.ZodNullable) {
        if (debug)
          currentLogger.debug(`[Convert Path: ${pathStr}] Empty/nullish string for ZodNullable. Returning null.`);
        return null;
      }
    }
    // If not empty/nullish, or if modifiers didn't result in a return, parse using the inner type
    if (debug)
      currentLogger.debug(`[Convert Path: ${pathStr}] Passing value to inner schema: ${innerSchema.constructor.name}`);
  ...
  if (schema instanceof z.ZodString) {
    ...
    const unescaped = stringValue
      ...
      .replace(/&apos;/g, "'");
    if (debug)
      currentLogger.debug(`[Convert Path: ${pathStr}] Returning string value: "${unescaped.substring(0, 100)}..."`);
    return unescaped;
    ...
    if (cleanValue === '') {
      ...
      if (debug)
        currentLogger.warn(`[Convert Path: ${pathStr}] Empty string reached number conversion. Returning default (0).`);
    ...
    if (debug)
      currentLogger.warn(
        `[Convert Path: ${pathStr}] Could not parse "${stringValue}" as boolean. Returning default (false).`,
      );
    ...
    const matchedEnum = enumValues.find(
      (enumVal) => typeof enumVal === 'string' && enumVal.toLowerCase() === lowerValue,
    );
    if (matchedEnum !== undefined) return matchedEnum;

    if (debug)
      currentLogger.warn(
        `[Convert Path: ${pathStr}] Value "${stringValue}" not in enum [${enumValues.join(', ')}]. Returning undefined.`,
      );
    ...
    if (debug)
      currentLogger.warn(
        `[Convert Path: ${pathStr}] Unhandled Zod type ${schema.constructor.name} in convertToPrimitive. Returning raw string value.`,
      );
    return stringValue;
  }
}
...
const last = <T>(arr: readonly T[]): T | undefined => (arr.length ? arr[arr.length - 1] : undefined);
...
function normalize(text: string): string {
  ...
  const commonModifiersRegex =
    /\s*\((?:optional|required|note|info|beta|new|deprecated|experimental|advanced|basic|default|example|eg|ie|important|warning|tip|hint|faq|todo|fixme|bug|issue|ref|see|compare|contrast|aka|viz|etc|misc|other)\)\s*$/i;
...
function extractPrimitiveValueFromNodes(
  nodes: MdNode[],
  debugInfo?: { path: string; logger: StructuredLogger; debug: boolean },
): string {
  const { path = '?', logger: currentLogger = logger, debug = false } = debugInfo || {};
  ...
  if (debug)
    currentLogger.debug(
      `[Extract Path: ${path}] Strategy 3: No code block or single list found. Stringifying relevant nodes.`,
    );
  const relevantNodes = nodes.filter(
    (node) => node.type === 'paragraph' || node.type === 'text' || node.type === 'inlineCode',
  ...
  if (stringified.startsWith('Description:')) {
    if (debug)
      currentLogger.debug(
        `[Extract Path: ${path}] Stringified value seems to be only the description. Returning empty string.`,
      );
    return '';
  }
...
function mapSectionToSchema(
  ...
  if (debug)
    logger.debug(
      `[Map Path: ${currentPath}] > Section: "${sectionHeadingText}", Target Schema: ${schema.constructor.name}`,
    );
  ...
  if (baseSchema instanceof z.ZodObject) {
    ...
    if (debug)
      logger.debug(`[Map Path: ${currentPath}] Initialized object with defaults`, {
        path: currentPath,
    ...
    if (debug)
      logger.debug(
        `[Map Path: ${currentPath}] Iterating through ${section.children.length} children of section "${sectionHeadingText}" to find matches...`,
      );
    for (const childSection of section.children) {
      if (childSection.heading) {
        ...
        if (debug)
          logger.debug(
            `[Map Path: ${currentPath}]  -- Checking child heading: "${childHeadingText}" (Normalized: "${normalizedChildHeading}")`,
          );
        ...
        for (const key of Object.keys(remainingKeys)) {
          ...
          const keyDistance =
            1.0 -
            getSemanticSimilarity(normalize(key), normalizedChildHeading, {
              useStemming: true,
              ngramMinSize: 1,
              ngramMaxSize: 2,
          ...
          if (debug)
            logger.debug(
              `[Map Path: ${currentPath}]   - Comparing "${key}" (Normalized: "${normalize(key)}") with "${normalizedChildHeading}". Distance: ${keyDistance}`,
            );
        ...
        if (matchedKey) {
          ...
          if (debug)
            logger.debug(
              `[Map Path: ${fieldPathStr}]   -- Normalized heading "${normalizedChildHeading}" matches schema key "${matchedKey}". Recursing.`,
            );
          // Recurse and update the output object, overwriting the default
          // Pass the specific field schema from the shape
          outputObject[matchedKey] = mapSectionToSchema(
            childSection,
            shape[matchedKey] as z.ZodTypeAny,
            fieldPath,
            debug,
            rootSchema,
          );
          ...
          if (debug)
            logger.debug(`[Map Path: ${currentPath}] Normalized heading does not match schema shape`, {
              path: currentPath,
    ...
    if (debug)
      logger.debug(`[Map Path: ${currentPath}] Finished processing children`, {
        path: currentPath,
        outputObject,
      });
    return outputObject;
  ...
  if (baseSchema instanceof z.ZodArray) {
    ...
    if (directList && !(itemSchema instanceof z.ZodObject)) {
      // Only use for primitive arrays
      if (debug)
        logger.debug(
          `[Map Path: ${currentPath}] Strategy 1: Found direct list in section nodes for primitive array (${directList.children.length} items).`,
        );
    ...
    if (debug)
      logger.debug(
        `[Map Path: ${currentPath}] Strategy 2: Looking for item sections matching regex: ${itemHeadingRegex}`,
      );
    ...
    if (debug)
      logger.debug(
        `[ItemMatching Path: ${itemMatchPath}] Searching in parent section "${sectionHeadingText}" with ${section.children.length} children.`,
      );
    ...
    for (const childSection of section.children) {
      if (childSection.heading) {
        ...
        if (debug)
          logger.debug(
            `[ItemMatching Path: ${itemMatchPath}]   - Checking child heading: "${headingText}". Regex match result: ${match}`,
          );
        if (match) {
    ...
    if (itemSections.length > 0) {
      if (debug)
        logger.debug(
          `[Map Path: ${currentPath}] Strategy 2: Found ${itemSections.length} item sections based on headings matching '${singularKeyName} X'.`,
        );
      return itemSections.map((itemSec, i) => {
        const itemPath = [...path, String(i)];
        if (debug)
          logger.debug(
            `[Map Path: ${itemPath.join('.')}] Mapping item section ${i} with heading "${mdastToString(itemSec.heading!)}".`,
          );
    ...
    if (slices.length > 0) {
      if (debug)
        logger.debug(`[Map Path: ${currentPath}] Strategy 3: Found ${slices.length} items based on thematic breaks.`);
    ...
    if (!(itemSchema instanceof z.ZodObject)) {
      ...
      const contentString = extractPrimitiveValueFromNodes(section.nodes, {
        path: currentPath,
        logger: logger as ReturnType<typeof createLogger>,
        debug,
      });
      if (!contentString) {
        if (debug)
          logger.debug(`[Map Path: ${currentPath}] No content found for primitive array. Returning empty array.`);
        return [];
      }
      ...
      if (looksLikeList) {
        if (debug)
          logger.debug(`[Map Path: ${currentPath}] Parsing primitive array from ${lines.length} list-like lines.`);
        return lines.map((ln, i) => {
          const itemPath = [...path, String(i)];
          const itemContent = ln.replace(/^\s*[-*+]\s+/, '').trim(); // Extract content from list item
          return convertToPrimitive(itemContent, itemSchema, {
            path: itemPath,
            logger: logger as ReturnType<typeof createLogger>,
            debug,
          });
        ...
        if (debug)
          logger.debug(
            `[Map Path: ${currentPath}] Parsing primitive array by splitting into ${lines.length} non-empty lines (non-list format).`,
          );
        return lines.map((ln, i) => {
          const itemPath = [...path, String(i)];
          return convertToPrimitive(ln, itemSchema, {
            path: itemPath,
            logger: logger as ReturnType<typeof createLogger>,
            debug,
          });
    ...
    if (debug)
      logger.debug(`[Map Path: ${currentPath}] No array items found using any strategy. Returning empty array.`);
    return [];
  }

  /* --- 2c. Primitive Types (and others like Union, Enum) --- */
  if (debug)
    logger.debug(`[Map Path: ${currentPath}] Handling Primitive/Other Schema (${baseSchema.constructor.name}).`);
  if (debug) {
    // Log nodes more concisely for primitives
    logger.debug(
      `[Map Path: ${currentPath}] Nodes for primitive extraction:`,
      section.nodes.map((n) => ({
        type: n.type,
        value: (n as any).value?.substring(0, 30) ?? mdastToString(n).substring(0, 30) + '...',
      })),
    );
  }
  // Extract the raw string value from the section's nodes
  const extractedValue = extractPrimitiveValueFromNodes(section.nodes, {
    path: currentPath,
    logger: logger as ReturnType<typeof createLogger>,
    debug,
  });
  if (debug)
    logger.debug(
      `[Map Path: ${currentPath}] Extracted raw value: "${extractedValue.substring(0, 100)}${extractedValue.length > 100 ? '...' : ''}"`,
    );
...
interface TemplateRegistryEntry {
  ...
  serialize: <OutputSchema extends z.AnyZodObject>(
    data: z.infer<OutputSchema>,
    schema: OutputSchema,
    options?: FormatterOptions,
  ) => string;
  prompt: (template: string) => string;
  parser: <OutputSchema extends z.AnyZodObject>(
    content: string,
    schema: OutputSchema,
    options?: FormatterOptions,
  ) => z.infer<OutputSchema>;
...
export function parseContentForSchema<OutputSchema extends z.AnyZodObject>(
  content: string,
  schemaFormat: SchemaFormat,
  schema: OutputSchema,
  options?: FormatterOptions,
