 ...
 */
function resolveModelConfig(agentActionName: AgentActionKey, userConfig?: ModelConfig): ModelConfig {
  ...
  const modelCandidates = [userConfig?.name, defaultConfig.name].filter((n): n is AIModels | string => n !== undefined);
...
export async function executeInference(params: InferenceParamsBase): Promise<InferResponseString>;
...
export async function executeInference<T extends z.AnyZodObject>({
  ...
  const resolvedConfig = resolveModelConfig(agentActionName, context?.userModelConfigs?.[agentActionName]);
