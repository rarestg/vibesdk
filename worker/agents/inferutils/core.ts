  ...
  if (deltaToolCall.function?.arguments !== undefined) {
    ...
    if (before.length > 0) {
      try {
        ...
        isComplete = true;
        console.warn(
          `[TOOL_CALL_WARNING] Already have complete JSON, ignoring additional chunk for ${entry.function.name}:`,
          {
...
function optimizeMessageContent(content: MessageContent): MessageContent {
  ...
  if (Array.isArray(content)) {
    return content.map((item) => (item.type === 'text' ? { ...item, text: optimizeTextContent(item.text) } : item));
  }
...
export async function infer<OutputSchema extends z.AnyZodObject>(
  ...
  try {
    ...
    if (stream) {
      ...
      const allToolCalls =
        ((response as OpenAI.ChatCompletion).choices[0]?.message
