...
export async function compactifyContext(
  ...
  try {
    ...
    const splitIndex = findTurnBoundary(runningHistory, COMPACTIFICATION_CONFIG.PRESERVE_RECENT_MESSAGES);
    ...
    const summary = await generateConversationSummary(messagesToSummarize, env, options, logger);
