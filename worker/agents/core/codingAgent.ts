...
export class CodeGeneratorAgent extends Agent<Env, AgentState> implements AgentInfrastructure<AgentState> {
  ...
  constructor(ctx: AgentContext, env: Env) {
    ...
    this.fileManager = new FileManager(stateManager, () => this.behavior?.getTemplateDetails?.() || null, this.git);
   ...
   */
  async initialize(initArgs: AgentInitArgs<AgentState>, ..._args: unknown[]): Promise<AgentState> {
  ...
  async onStart(props?: Record<string, unknown> | undefined): Promise<void> {
    ...
    if (!this.state.query) {
      this.logger().warn(
        `Agent ${this.getAgentId()} session: ${this.state.sessionId} onStart ignored, agent not initialized`,
      );
      return;
    }
    ...
    this.logger().info(
      `Agent ${this.getAgentId()} session: ${this.state.sessionId} onStart: User configs loaded successfully`,
      { userConfigsRecord },
    );
  }
  ...
  async getDecryptedSecret(query: {
    provider?: string;
    envVarName?: string;
    secretId?: string;
  }): Promise<string | null> {
  ...
  getConversationState(id: string = DEFAULT_CONVERSATION_SESSION_ID): ConversationState {
    ...
    const compactRows = this.sql<{
      messages: string;
      id: string;
    }>`SELECT * FROM compact_conversations WHERE id = ${id}`;
  ...
  setConversationState(conversations: ConversationState) {
    ...
    try {
      ...
      void this
        .sql`INSERT OR REPLACE INTO compact_conversations (id, messages) VALUES (${conversations.id}, ${serializedCompact})`;
      void this
        .sql`INSERT OR REPLACE INTO full_conversations (id, messages) VALUES (${conversations.id}, ${serializedFull})`;
    } catch (error) {
  ...
  async handleBrowserFileServing(request: Request): Promise<Response> {
    ...
    if (lastHyphenIndex === -1) {
      this.logger().warn('[BROWSER SERVING] Invalid hostname pattern - no hyphen after prefix', {
        hostname: url.hostname,
      });
      return new Response('Invalid request', {
        status: 400,
    ...
    const filePath =
      url.pathname === '/' || url.pathname === '' ? 'public/index.html' : url.pathname.replace(/^\//, ''); // Remove leading slash
