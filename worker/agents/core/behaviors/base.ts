...
import { FileConceptType, FileOutputType, Blueprint, AgenticBlueprint, PhasicBlueprint } from '../../schemas';
import {
  ...
  implements ICodingAgent
{
  ...
  public async initialize(initArgs: AgentInitArgs, ..._args: unknown[]): Promise<TState> {
  ...
  protected isPreviewable(): boolean {
    ...
    return (
      ...
      (this.fileManager.fileExists('wrangler.jsonc') || this.fileManager.fileExists('wrangler.toml'))
    );
  }
  ...
  private async updateBootstrapScript(commandsHistory: string[]): Promise<void> {
    ...
    const bootstrapScript = generateBootstrapScript(this.state.projectName, commandsHistory);
    ...
    try {
      ...
      if (fixResult) {
        ...
        if (fixResult.unfixableIssues.length > 0) {
          ...
          const moduleNames = modulesNotFound
            .flatMap((issue) => {
  ...
  async fetchAllIssues(resetIssues: boolean = false): Promise<AllIssues> {
    if (!this.state.sandboxInstanceId) {
      return {
        runtimeErrors: [],
        staticAnalysis: { success: false, lint: { issues: [] }, typecheck: { issues: [] } },
      };
    }
  ...
  async readFiles(paths: string[]): Promise<{ files: { path: string; content: string }[] }> {
    ...
    if (notFoundInFileManager.length > 0 && this.state.sandboxInstanceId) {
      const resp = await this.getSandboxServiceClient().getFiles(this.state.sandboxInstanceId, notFoundInFileManager);
      if (resp.success) {
        results.push(
          ...resp.files.map((f) => ({
            path: f.filePath,
  ...
  async generateFiles(
    ...
    if (templateDetails && templateDetails.dontTouchFiles) {
      ...
      files = files.filter((file) => {
        if (dontTouchFiles.has(file.path)) {
          ...
          skippedFiles.push({
            path: file.path,
            purpose: `WRITE-PROTECTED FILE, CANNOT MODIFY`,
            diff: '<WRITE PROTECTED - TEMPLATE FILE, CANNOT MODIFY - SKIPPED - NO CHANGES MADE>',
          });
          return false;
        }
    ...
    await this.fileManager.saveGeneratedFiles([], `feat: ${phaseName}\n\n${phaseDescription}`);
  ...
  async deployToSandbox(
    ...
    const result = await this.deploymentManager.deployToSandbox(files, redeploy, commitMessage, clearLogs, {
      onStarted: (data) => {
  ...
  protected async saveExecutedCommands(commands: string[]) {
    ...
    const hasDependencyCommands = commands.some(
      (cmd) => cmd.includes('install') || cmd.includes(' add ') || cmd.includes('remove') || cmd.includes('uninstall'),
    );
      ...
      while (currentChunk.length > 0 && retryCount < maxRetries) {
        try {
          this.broadcast(WebSocketMessageResponses.COMMAND_EXECUTING, {
            message:
              retryCount > 0 ? `Retrying commands (attempt ${retryCount + 1}/${maxRetries})` : 'Executing commands',
            commands: currentChunk,
          });

          const resp = await this.getSandboxServiceClient().executeCommands(state.sandboxInstanceId, currentChunk);
   ...
   */
  private async executeScreenshotCapture(url: string, viewport: { width: number; height: number }): Promise<string> {
