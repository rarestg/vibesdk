...
export class PhasicCodingBehavior extends BaseCodingBehavior<PhasicState> implements ICodingAgent {
   ...
   */
  async initialize(initArgs: AgentInitArgs<PhasicState>, ..._args: unknown[]): Promise<PhasicState> {
    ...
    const customizedFiles = customizeTemplateFiles(templateInfo.templateDetails.allFiles, {
      projectName,
      commandsHistory: [],
    });
    ...
    await this.fileManager.saveGeneratedFiles(filesToSave, 'Initialize project configuration files', true);
  ...
  async executePhaseImplementation(
    ...
    try {
      ...
      if ((phaseConcept.lastPhase || phasesCounter <= 0) && this.state.pendingUserInputs.length === 0)
        return { currentDevState: CurrentDevState.FINALIZING };
  ...
  getTotalFiles(): number {
    return (
      ...
      ((this.state.currentPhase || this.state.blueprint.initialPhase)?.files?.length || 0)
    );
  }
