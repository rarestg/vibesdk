...
export function handleWebSocketMessage(agent: CodeGeneratorAgent, connection: Connection, message: string): void {
  try {
    ...
    switch (parsedMessage.type) {
      ...
      case WebSocketMessageRequests.CAPTURE_SCREENSHOT:
        agent
          .getBehavior()
          .captureScreenshot(parsedMessage.data.url, parsedMessage.data.viewport)
      ...
      case WebSocketMessageRequests.STOP_GENERATION: {
        ...
        sendToConnection(connection, WebSocketMessageResponses.GENERATION_STOPPED, {
          message: wasCancelled ? 'Inference operation cancelled successfully' : 'No active inference to cancel',
        });
        break;
      }
