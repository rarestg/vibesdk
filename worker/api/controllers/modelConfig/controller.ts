...
export class ModelConfigController extends BaseController {
   ...
   */
  static async getModelConfigs(
    _request: Request,
    env: Env,
    _ctx: ExecutionContext,
    context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<ModelConfigsData>>> {
    try {
  ...
  static async getModelConfig(
    ...
    _ctx: ExecutionContext,
    context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<ModelConfigData>>> {
    try {
      const user = context.user!;
  ...
  static async updateModelConfig(
    ...
    _ctx: ExecutionContext,
    context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<ModelConfigUpdateData>>> {
    try {
      const user = context.user!;
      ...
      if (!modelConfig.name) {
        return ModelConfigController.createErrorResponse<ModelConfigUpdateData>('Model name is required', 400);
      }
   ...
   */
  static async deleteModelConfig(
    request: Request,
    env: Env,
    _ctx: ExecutionContext,
    context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<ModelConfigDeleteData>>> {
    try {
      const user = context.user!;
      ...
      if (!deleted) {
        return ModelConfigController.createErrorResponse<ModelConfigDeleteData>(
          'Configuration not found or already using defaults',
          404,
        );
      }
   ...
   */
  static async testModelConfig(
    request: Request,
    env: Env,
    _ctx: ExecutionContext,
    context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<ModelConfigTestData>>> {
    try {
      const user = context.user!;
      ...
      const configToTest: ModelConfig = validatedData.tempConfig
        ? {
            ...
            ...(validatedData.tempConfig.maxTokens != null && {
              max_tokens: validatedData.tempConfig.maxTokens,
            }),
            ...(validatedData.tempConfig.temperature != null && {
              temperature: validatedData.tempConfig.temperature,
            }),
            ...(validatedData.tempConfig.reasoningEffort != null && {
              reasoning_effort: validatedData.tempConfig.reasoningEffort,
            }),
            ...(validatedData.tempConfig.fallbackModel != null && {
              fallbackModel: validatedData.tempConfig.fallbackModel,
            }),
            ...(validatedData.tempConfig.providerOverride != null && {
              providerOverride: validatedData.tempConfig.providerOverride,
            }),
          }
        : baseConfig;
      ...
      const responseData: ModelConfigTestData = {
        testResult,
        message: testResult.success ? 'Model configuration test successful' : 'Model configuration test failed',
      };

      return ModelConfigController.createSuccessResponse(responseData);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return ModelConfigController.createErrorResponse<ModelConfigTestData>(
          'Validation failed: ' + JSON.stringify(error.errors),
          400,
        );
      }
  ...
  static async resetAllConfigs(
    ...
    _ctx: ExecutionContext,
    context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<ModelConfigResetData>>> {
    try {
      const user = context.user!;
      ...
      return ModelConfigController.createErrorResponse<ModelConfigResetData>(
  ...
  static async getDefaults(
    ...
    env: Env,
    _ctx: ExecutionContext,
  ): Promise<ControllerResponse<ApiResponse<ModelConfigDefaultsData>>> {
    try {
      const modelConfigService = new ModelConfigService(env);
      const defaults = modelConfigService.getDefaultConfigs();
      ...
      return ModelConfigController.createErrorResponse<ModelConfigDefaultsData>(
        'Failed to get default configurations',
        500,
      );
    }
  }
   ...
   */
  static async getByokProviders(
    request: Request,
    env: Env,
    _ctx: ExecutionContext,
    context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<ByokProvidersData>>> {
    try {
      const user = context.user!;
