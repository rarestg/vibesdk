...
import { AgentConnectionData, AgentPreviewResponse, CodeGenArgs, MAX_AGENT_QUERY_LENGTH } from './types';
...
export class CodingAgentController extends BaseController {
   ...
   */
  static async startCodeGeneration(
    request: Request,
    env: Env,
    _: ExecutionContext,
    ...
    try {
      ...
      try {
        ...
        return CodingAgentController.createErrorResponse(
          `Invalid JSON in request body: ${JSON.stringify(error, null, 2)}`,
          400,
        );
      }
      ...
      try {
        ...
        if (!agentInstance || !(await agentInstance.isInitialized())) {
          return CodingAgentController.createErrorResponse<AgentConnectionData>(
            'Agent instance not found or not initialized',
            404,
          );
        }
        ...
        return CodingAgentController.createErrorResponse<AgentConnectionData>(
          `Agent instance not found or unavailable: ${error instanceof Error ? error.message : String(error)}`,
          404,
        );
      }
    } catch (error) {
      this.logger.error('Error connecting to existing agent', error);
      return CodingAgentController.handleError(error, 'connect to existing agent') as ControllerResponse<
        ApiResponse<AgentConnectionData>
      >;
    }
  }
  ...
  static async deployPreview(
    ...
    try {
      ...
      if (appResult.visibility !== 'public') {
        ...
        if (!user || user.id !== appResult.userId) {
          return CodingAgentController.createErrorResponse<AgentPreviewResponse>(
            'App is not public. Preview deployment is only available for public apps.',
            403,
          );
        }
      }
      ...
      const appError = CodingAgentController.handleError(error, 'deploy preview') as ControllerResponse<
        ApiResponse<AgentPreviewResponse>
      >;
      return appError;
    }
  }
