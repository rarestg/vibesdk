...
export class AppController extends BaseController {
  ...
  ): Promise<ControllerResponse<ApiResponse<AppsListData>>> {
    try {
  ...
  static async toggleFavorite(
    ...
    _ctx: ExecutionContext,
    context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<FavoriteToggleData>>> {
    try {
      const user = context.user!;
  ...
  static getPublicApps = async function (
    this: AppController,
    request: Request,
    env: Env,
    _ctx: ExecutionContext,
    _context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<PublicAppsData>>> {
    try {
  ...
  static async getApp(
    ...
    _ctx: ExecutionContext,
    context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<SingleAppData>>> {
    try {
      const user = context.user!;
  ...
  static async updateAppVisibility(
    request: Request,
    env: Env,
    _ctx: ExecutionContext,
    context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<UpdateAppVisibilityData>>> {
    try {
      const user = context.user!;
      ...
      if (!visibility || !['private', 'public'].includes(visibility)) {
        return AppController.createErrorResponse<UpdateAppVisibilityData>(
      ...
      if (!result.success) {
        const statusCode =
          result.error === 'App not found'
            ? 404
            : result.error?.includes('only change visibility of your own apps')
              ? 403
              : 500;
        return AppController.createErrorResponse<UpdateAppVisibilityData>(
          result.error || 'Failed to update app visibility',
          statusCode,
        );
      }
  ...
  static async deleteApp(
    ...
    _ctx: ExecutionContext,
    context: RouteContext,
  ): Promise<ControllerResponse<ApiResponse<AppDeleteData>>> {
    try {
      const user = context.user!;
      ...
      if (!result.success) {
        const statusCode =
          result.error === 'App not found' ? 404 : result.error?.includes('only delete your own apps') ? 403 : 500;
