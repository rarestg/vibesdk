...
async function verifyGitAccess(
  ...
  logger.info('Private app - checking auth header', {
  ...
  if (authHeader?.startsWith('Bearer ')) {
    ...
    logger.info('Extracted Basic auth token', {
 ...
 */
async function handleInfoRefs(request: Request, env: Env, ctx: ExecutionContext, appId: string): Promise<Response> {
  try {
    // Verify access first
 ...
 */
async function handleUploadPack(request: Request, env: Env, _ctx: ExecutionContext, appId: string): Promise<Response> {
  try {
    // Verify access first
    const { hasAccess, appCreatedAt } = await verifyGitAccess(request, env, appId);
    if (!hasAccess) {
 ...
 */
export async function handleGitProtocolRequest(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
